var Shift = function() {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			if (date == elem.date.substr(0, 10)) {					
				row += "<tr>" +
						"<td><span>"+ elem.date.substr(0, 10) +"</span></td>" +
						"<td><span>"+ elem.dcno +"</span></td>" +
						"<td><span>"+ elem.status +"</span></td>" +
						"<td><span>"+ elem.description +"</span></td>" +
					  "</tr>";
			}
		});
	...
	var getSaveObject = function() {
		...
		var atnd = {
			...
			status : _status,
			date : _date,
			description : _description + " - Updated",
			tin : _tin,
			tout : _tout,
			uid : _uid,
	...
	var validateAttendance = function () {
		...
		var date = $('#current_date').val().split('/').join('-');
		var found = false;
		$('#atnd-table tbody tr').each(function(index, elem) {

			var _date = $(elem).find('td').eq(0).text().trim();
			if (date == _date) {				
				found = true;
				return false;
			}
