var StaffAttendance = function() {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			oTable.fnAddData( [
				...
				"<span class='fname' >"+ elem.fname +"</span>",
				...
				"<span class='description'>"+ ((elem.description)? elem.description : '') +"</span>",
				"<input type='text' class='tableInputCell atnd-status' list='status' value='"+ status +"'/>", 
				"<span class='tin'>"+ ((elem.tin)? elem.tin : '') +"</span>",
				"<span class='tout'>"+ ((elem.tout)? elem.tout : '') +"</span>",
				"<span class='late'>"+ ((elem.late)? elem.late : '') +"</span>",
				"<span class='wh'>"+ ((elem.wh)? elem.wh : '') +"</span>"]
				);
		});
	}
	...
	var getSaveObject = function() {
		...
		$('#atnd-table').find('tbody tr').each(function(index, elem) {
			...
			var _description = $.trim($(this).closest('tr').find('span.description').text());
			
			var _tin = $.trim($(this).closest('tr').find('span.tin').text());
			var _tout = $.trim($(this).closest('tr').find('span.tout').text());
			var _wh = $.trim($(this).closest('tr').find('span.wh').text());
			var _late = $.trim($(this).closest('tr').find('span.late').text());
	...
	var print = function() {
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			window.open(base_url + 'application/views/payrollprints/attendancevoucher.php', 'Attendance Voucher', 'width=720, height=850');
		}
	}
