var FabricPurchaseContract = function() {
	var settings = {
	...
	var save = function(sale) {
		...
		$.ajax({
			url : base_url + 'index.php/fabricPurchaseContract/save',
			type : 'POST',
	...
	var Print_Voucher = function(hd ) {
		...
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			var etype=  'fabricPurchaseContract';
	...
	var fetchItemStocks = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'fabricPurchaseContract'},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
					// alert('No data found.');
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/fabricPurchaseContract/fetch',
			type : 'POST',
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/fabricPurchaseContract/getMaxVrno',
			type : 'POST',
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/fabricPurchaseContract/getMaxVrnoa',
			type : 'POST',
			data : {'company_id': $('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id,uom,pick,reed,width,qty,count, rate, amount, weight,workorderno) {
		...
		var row = 	"<tr>" +
						"<td class='srno numeric text-right' data-title='Sr#' > "+ srno +"</td>" +
				 		...
				 		"<td class='count numeric text-right hide' data-title='Count'>  "+ count +"</td>" +
					 	"<td class='pick text-right hide' data-title='Pick'>  "+ pick +"</td>" +
					 	"<td class='reed text-right hide' data-title='Reed'>  "+ reed +"</td>" +
					 	"<td class='width text-right hide' data-title='Width'>  "+ width +"</td>" +
...
var getSaveObjectItem = function() {
	...
	var getSaveObject = function() {
		...
		stockmain.etype 		= 'fabricPurchaseContract';
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/fabricPurchaseContract/delete',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'fabricPurchaseContract','company_id':$('#cid').val()},//, 'company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var calculateLowerTotal = function(qty, amount, weight) {
		...
		$('.txtTotalQty').text((tempQty).toFixed(2));
		var tempAmnt = parseFloat(_amnt) + parseFloat(amount);
		$('.txtTotalAmount').text((tempAmnt).toFixed(2));
   ...
   //  			$('#ItemAddModel').modal('show');
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					var reed = $('#txtReed').val();
					var count = $('#txtCount').val('');
					var qtygross  		= $('#txtQtyGross').val();
					var lkami  		= $('#txtLKami').val();
					...
					appendToTable('', item_desc, item_id,uom,lkami,reed,width,qty,count, rate, amount, weight,workorderno);
...
var fabric = new FabricPurchaseContract();
fabric.init();