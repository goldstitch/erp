var Transfer = function() {
	...
	var fetchReports = function (from, to,companyid,etype,uid) {
		...
		if (typeof dTable != 'undefined') {
		    ...
		    var appendToTable = function(srno, item_desc, item_id, deptfrom, deptfrom_id, uom, qty,receive, weight, deptto, deptto_id, dozen) {
		    	...
		    	"<td class='qty' id= data-title='Qty' style='text-align:right;'> "+ receive +"</td>" +
	...
	var calculateNetQty = function(qty, weight ,  dozen) {
		...
		$('#txtGQty').text(parseFloat(tempWeight).toFixed(2));
	...
	var fetchLfiveStocks = function(item_id) {
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': 1,'etype': 'purchase' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
		...
		bindUI : function() {
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					var uom = $('#txtUOM').val();
					var qty = $('#txtRec').val();
					var receive = $('#txtSQty').val();
					var dozen = '';
					var weight = $('#balance').val();
					var deptto = $('#deptto_dropdown').find('option:selected').text();
					var deptto_id = $('#deptto_dropdown').val();
					var item_id = $('#hfItemId').val(); 
					var deptfrom_id = $('#deptfrom_dropdown').val();
					var deptfrom = $('#deptfrom_dropdown').find('option:selected').text();
					appendToTable('1', item_desc, item_id, deptfrom, deptfrom_id, uom, qty,receive, weight, deptto, deptto_id, dozen);
