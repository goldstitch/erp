var settelmentvendor = function() {
	...
	var fetchLfiveStocks = function(item_id) {
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'settelment' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'settelment','crit':crit,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLvendor = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {  'company_id': $('#cid').val(),'etype': 'settelment','crit':crit,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
...
var populateDataGodowns = function(data) {
	...
	var save = function(purchase) {
		...
		$.ajax({
			url : base_url + 'index.php/settelmentvendors/save',
			type : 'POST',
			data : { 'stockmain' : purchase.stockmain, 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa, 'voucher_type_hidden':$('#voucher_type_hidden').val(), 'ledger' : purchase.ledger },
			dataType : 'JSON',
			success : function(data) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/settelmentvendors/fetch',
			type : 'POST',
	...
	var populateData = function(data) {
		...
		$('#party_dropdown11').select2('val', data['main'][0]['party_id']);
		$('#txtInvNo').val(data['main'][0]['inv_no']);
		...
		$('#txtOrderNo').val(data['main'][0]['workorder']);
		...
		$('#txtPaid').val(data['main'][0]['paid']);

		
		$('#voucher_type_hidden').val('edit');		
		$('#user_dropdown').val(data['main'][0]['uid']);

		$('#due_date').val(data['main'][0]['due_date'].substring(0,10));
		$('#bilty_date').val(data['main'][0]['bilty_date'].substring(0,10));
		
		$('#txtOpening').val(data['main'][0]['bilty_no']);
		$('#txtIn').val(data['main'][0]['order_no']);
		$('#txtOut').val(data['main'][0]['freight']);
		$('#txtBalance').val(data['main'][0]['salebillno']);
		$('#txtStichAmount').val(data['main'][0]['weight']);


		$.each(data['detail'], function(index, elem) {
			$('#dept_dropdown').select2('val', elem.godown_id);
			if(elem.type!=='less'){
				appendToTable('', elem.item_name, elem.item_id,elem.phase_name,elem.phase_id,elem.s_qty,elem.s_rate, elem.s_amount, elem.weight,elem.dozen,elem.bag);
				calculateLowerTotal(elem.s_qty, elem.s_amount, elem.weight,elem.dozen,elem.bag);
			}
		});
	}
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/settelmentvendors/getMaxVrno',
			type : 'POST',
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/settelmentvendors/getMaxVrnoa',
			type : 'POST',
			data : {'company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var getSaveObject = function() {
		...
		stockmain.ddate = $('#due_date').val();
		stockmain.bilty_date = $('#bilty_date').val();
		
		stockmain.bilty_no = $('#txtOpening').val();
		stockmain.order_vrno = $('#txtIn').val();
		stockmain.freight = $('#txtOut').val();
		stockmain.salebillno = $('#txtBalance').val();
		stockmain.weight = $('#txtStichAmount').val();
		...
		///////////////////////////////////////////////////////////////
		if ($('#txtNetAmount').val() != 0 || $('#txtNetAmount').val() != '') {
			var pledger = {};
			pledger.pledid = '';
			pledger.pid = $('#party_dropdown11').val();
			pledger.description = 'from: ' + $('#due_date').val() + ' -To- ' + $('#bilty_date').val() + ', ' + $('#txtRemarks').val();
			...
			pledger.date = $('#current_date').val();
			pledger.debit = getNumVal($('#txtStichAmount'))+getNumVals($('#txtTotalAmount'));
			pledger.credit = 0;
			pledger.dcno = $('#txtVrnoaHidden').val();
...
var Validate_Stock = function(item_id,edit_qty,qty_chk,edit_weight,weight_chk,order_no,uom) {
		var chk=false;
		if(uom=='dozen'){
			qty_chk= qty_chk/12;
		}
		uom= uom.toLowerCase();
		$('.Lvendors_table').find('tbody tr').each(function( index, elem ) {
			var location_order_no = $.trim($(elem).find('td.workorder').text());
			
			if (location_order_no==order_no){
	...
	var validateGetStichData = function() {
		...
		if ( !partyEl.val() ) {
			...
			$('#party_dropdown11').addClass('inputerror');
			errorFlag = true;
			alert('Please select valid party............');
		}
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/settelmentvendors/delete',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'settelment','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var calculateLowerTotal = function(qty, amount, weight,dozen,bag) {
		...
		var _stich = getNumVal($('#txtStichAmount'));
		...
		var net = parseFloat(tempAmnt) +parseFloat(_stich) - parseFloat(_taxamount) - parseFloat(_expense) ;
	...
	var fetchSettelmentData = function() {
		var error = validateGetStichData();
		if(!error){
			$.ajax({
				url : base_url + 'index.php/settelmentvendors/fetch_settellement',
				type : 'POST',
				data : { 'pid' : $('#party_dropdown11').val(),'endDate': $('#bilty_date').val(),'startDate': $('#due_date').val() , 'company_id': $('#cid').val()},
				dataType : 'JSON',
				success : function(data) {
					// resetFields();
					...
					if (data === 'false') {
						alert('No data found.');
					}else if(data[0]['duplicate_vrno']!=='00'){
						alert('Duplicate settelment found: Already Save till: ' + data[0]['bilty_date'].substring(0,10) + ', Vr# '+ data[0]['duplicate_vrno']  );
					} else {
						populateStitchData(data);
					}
	...
	var populateStitchData = function(data) {
			console.log(data);
		$('#txtOpening').val(parseFloat(data[0]['opqty']).toFixed(0));
		$('#txtIn').val(parseInt(data[0]['inqty']))  ;
		$('#txtOut').val(parseInt(data[0]['outqty']));
		$('#txtBalance').val(parseInt(data[0]['balqty']));
		$('#txtStichAmount').val(parseInt(data[0]['amount']));
		...
		bindUI : function() {
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					var uom = $('#txtUom').val();
					// var error_stk = Validate_Stock(item_id,edit_qty,qty,edit_weight,weight,order_no,uom);
					// if(error_stk){
			...
			$('#txtDiscount').on('input', function() {
				...
				var _totalAmount= $('#txtIn').val();
				var _discamount=0;
				if (_disc!=0 && _totalAmount!=0){
					_discamount=_totalAmount*_disc/100;
				}
				$('#txtDiscAmount').val(parseFloat(_discamount).toFixed(2));
				calculateLowerTotal(0, 0, 0,0,0);
			});
			...
			$('#txtExpense').on('input', function() {
				...
				var _totalAmount= $('#txtDiscAmount').val();
				var _expamount=0;
				if (_exppercent!=0 && _totalAmount!=0){
					_expamount=_totalAmount*_exppercent;
				}
			...
			$('#txtTax').on('input', function() {
				...
				var _totalAmount= $('#txtStichAmount').val();
				var _taxamount=0;
...
var issue = new settelmentvendor();
issue.init();