var Receivefromvender = function() {
	...
	var save = function(purchase) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'stockmain' : purchase.stockmain , 'vendordetail' : JSON.stringify(purchase.vendordetail), 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa, 'voucher_type_hidden':$('#voucher_type_hidden').val(), 'ledger' : purchase.ledger ,'etype':'production' },
			dataType : 'JSON',
			success : function(data) {
	...
	var CheckDuplicateVoucher = function(stockmain) {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'gp' : $('#txtBilty').val(), 'vrnoa' : $('#txtVrnoaHidden').val(),'etype':'production', 'company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
				console.log(data);
				if (data === false) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val() ,'etype': 'production' },
			dataType : 'JSON',
			success : function(data) {
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'production'},
			dataType : 'JSON',
			success : function(data) {
...
var resetFields = function() {
	...
	var FetchBulkItemsSearch = function () {
		$.ajax({
			...
			type: 'POST',
			data: {'vrnoa': $('#txtOrderNo').val(), 'company_id':$('#cid').val()},
			dataType: 'JSON',
			success: function (data) {
				if (data === 'false') {
					alert('No data found');
	...
	var getcritBulk = function (etype){
		...
		var vrnoa=$('#article_dropdown').select2("val");
		...
		if (vrnoa!='') {
			crit +='AND d.godown_id2 in (' + vrnoa +') ';
		}
	...
	var populateDataIrm = function(data) {
		...
		$.each(data, function(index, elem) {
			if (elem.detype === 'material' || elem.detype === 'packing' || elem.detype === 'fabric' ) {
				...
				qty = parseFloat(parseFloat(elem.bundle) * parseFloat(pro_qty)).toFixed(2) ;
				stk = (elem.st_qty) ? elem.st_qty : 0;
				amount= parseFloat(parseFloat(qty)*parseFloat(elem.item_avg_rate)).toFixed(0);
				
				if(parseFloat(qty)>parseFloat(stk))
					others='Stock Qty Not Available';



				appendToTableConsumed('', elem.item_name, elem.item_id,'','', qty,parseFloat(elem.item_avg_rate).toFixed(0),amount, weight,elem.uom,others,elem.item_inventory_id,godown_id,godown_name);
				// appendToTableConsumed = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight, uom,others,inventory_id,godown_id,godown_name)
	...
	return {
		...
		bindUI : function() {
			...
			$('.btnBulkItemsSearch').on('click',function(e){
				...
				if(parseInt(vrnoa)==0 || vrnoa==''){
					
					$('#txtOrderNo').addClass('inputerror');
					alert('Please choose valid work order........');
				...
				if(parseInt(godown_id)==0 || godown_id=='' || godown_id==null ){
					
					$('#dept_dropdown_cons').addClass('inputerror');
					alert('Please choose valid consumed stock location........');
					$('#bulkitem-lookup').modal('hide');
					return false;
				}
				if(parseFloat(qty)==0 ){
					...
					alert('Please choose valid produced qty........');
					$('#bulkitem-lookup').modal('hide');
					return false;
				}


				$('#txtworkOrderNo2').val(vrnoa);
