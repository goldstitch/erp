...
var populateDataGodowns = function(data) {
	...
	var save = function(purchase) {
		...
		$.ajax({
			url : base_url + 'index.php/glovescontract/save',
			type : 'POST',
			data : { 'stockmain' : purchase.stockmain, 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa, 'etype':'glovescontract','voucher_type_hidden':$('#voucher_type_hidden').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var Print_Voucher = function(hd,prnt,wrate) {
		...
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			// var url = base_url + 'index.php/doc/Print_Order_Voucher/' + etype + '/' + vrnoa + '/' + company_id + '/' + '-1' + '/' + user + '/' + pre_bal_print + '/' + hd + '/' + prnt;
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/glovescontract/fetch',
			type : 'POST',
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			appendToTable('', elem.item_name, elem.item_id,elem.phase_name,elem.phase_id,Math.abs(elem.qty),elem.rate, elem.amount, Math.abs(elem.weight),elem.dozen,elem.bag,elem.frate,elem.lprate,elem.qtyf,elem.bundle,elem.item_desc_cus, elem.item_id_cus,elem.phaseTo,elem.phase_id2,elem.grossweight);
			// appendToTable('', item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dozen,bag,req_weight,bag_rate,wastage,bag_weight,item_desc1, item_id1,phase1,phase_id1 ,grossweight);
			calculateLowerTotal(Math.abs(elem.qty), elem.amount, Math.abs(elem.weight),elem.dozen,elem.bag);
		});
	}
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/glovescontract/getMaxVrno',
			type : 'POST',
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/glovescontract/getMaxVrnoa',
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'glovescontract'},
			dataType : 'JSON',
			success : function(data) {
	...
	var validateSingleProductAdd = function() {
		...
		var item1El = $('#item_dropdown1');
		var phase1 = $('#phase_dropdown1');
	...
	var appendToTable = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dzn_qty,bag,frate,lprate,qtyf,bundle,item_desc1, item_id1,phase1,phase_id1 ,grossweight) {
		...
		var row = 	"<tr>" +
						"<td class='srno numeric' data-title='Sr#' > "+ srno +"</td>" +
				 		...
				 		"<td class='frate numeric' data-title='Req Wt' style='text-align:right;'>  "+ frate +"</td>" +
				 		"<td class='wastage numeric' data-title='Wastage%' style='text-align:right;'>  "+ qtyf +"</td>" +
				 		"<td class='lprate numeric' data-title='Bag Rt' style='text-align:right;'>  "+ lprate +"</td>" +
					 	"<td class='bundle numeric' data-title='Bag Wt' style='text-align:right;'>  "+ bundle +"</td>" +
					 	...
					 	"<td class='item_desc1' data-title='Description' data-item_id1='"+ item_id1 +"'> "+ item_desc1 +"</td>" +
				 		"<td class='phase1' data-title='Phase1' data-phase_id1='"+ phase_id1 +"'> "+ phase1 +"</td>" +
				 		"<td class='grossweight numeric text-right' data-title='Gross Weight'>  "+ grossweight +"</td>" +
	...
	var getSaveObject = function() {
		...
		stockmain.workorderno = $('#txtOrderNo').val();
		...
		$('#purchase_table').find('tbody tr').each(function( index, elem ) {
			...
			sd.qtyf = $.trim($(elem).find('td.wastage').text());
			sd.lprate = $.trim($(elem).find('td.lprate').text());
			sd.bundle = $.trim($(elem).find('td.bundle').text());
			sd.item_id_cus = $.trim($(elem).find('td.item_desc1').data('item_id1'));
			sd.phase_id2 = $.trim($(elem).find('td.phase1').data('phase_id1'));
			sd.grossweight = $.trim($(elem).find('td.grossweight').text());
...
var Validate_Stock = function(item_id,edit_qty,qty_chk,edit_weight,weight_chk,godown,uom) {
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/glovescontract/delete',
			type : 'POST',
	...
	var calculateUpperSum = function() {
		...
		if(_uom === 'dozen' ){
			...
			var _weight = parseFloat(_qty) * parseFloat(_rweight);
		}
        ...
        var wastageR = parseFloat(_weight) * parseFloat(_wastage) / 100;
        var totalWeight = parseFloat(_weight) + parseFloat(wastageR);
	...
	return {
		...
		bindUI : function() {
			...
			$('#txtGrossWeight').on('keypress', function(e) {
				if (e.keyCode === 13) {
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					var req_weight = $('#txtReqWeight').val();
					var wastage = $('#txtWastage').val();
					var bag_weight = $('#txtBagWeight').val();
					var bag_rate = $('#txtBagRate').val();

					var item_desc1 = $('#item_dropdown1').find('option:selected').text();
					var item_id1 = $('#item_dropdown1').val();
					var phase1 = $('#phase_dropdown1').find('option:selected').text();
					var phase_id1 = $('#phase_dropdown1').val();
			...
			$('#purchase_table').on('click', '.btnRowRemove', function(e) {
				...
				var req_weight =$.trim($(this).closest('tr').find('td.frate').text());
				var wastage =$.trim($(this).closest('tr').find('td.wastage').text());
				var bag_weight =$.trim($(this).closest('tr').find('td.bundle').text());
				var bag_rate =$.trim($(this).closest('tr').find('td.lprate').text());
			...
			$('#purchase_table').on('click', '.btnRowEdit', function(e) {
				...
				var phase_id1 = $.trim($(this).closest('tr').find('td.phase1').data('phase_id1'));

				var req_weight =$.trim($(this).closest('tr').find('td.frate').text());
				var wastage =$.trim($(this).closest('tr').find('td.wastage').text());
				var bag_weight =$.trim($(this).closest('tr').find('td.bundle').text());
				var bag_rate =$.trim($(this).closest('tr').find('td.lprate').text());
				var grossweight =$.trim($(this).closest('tr').find('td.grossweight').text());
				...
				var uom_item = $('#item_dropdown').find('option:selected').data('uom_item');
				 ...
				 $('#txtReqWeight').val(req_weight);
				 $('#txtWastage').val(wastage);
				 $('#txtBagWeight').val(bag_weight);
				 $('#txtBagRate').val(bag_rate);
				...
				$('#phase_dropdown1').select2('val', phase_id1);
				$('#txtUom').val(uom_item);
				calculateLowerTotal("-"+qty, "-"+amount, '-'+weight , '-'+dozen,'-'+bag);
