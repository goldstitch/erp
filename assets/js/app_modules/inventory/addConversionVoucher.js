var Receivefromvender = function() {
	...
	var fetchLfiveStocks = function(item_id) {
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'conversion' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'conversion','crit':crit},
			dataType : 'JSON',
			success : function(data) {
	...
	var save = function(purchase) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'stockmain' : purchase.stockmain , 'vendordetail' : JSON.stringify(purchase.vendordetail), 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa, 'voucher_type_hidden':$('#voucher_type_hidden').val(), 'ledger' : purchase.ledger ,'etype':'conversion' },
			dataType : 'JSON',
			success : function(data) {
	...
	var CheckDuplicateVoucher = function(stockmain) {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'gp' : $('#txtBilty').val(), 'vrnoa' : $('#txtVrnoaHidden').val(),'etype':'conversion', 'company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
				console.log(data);
				if (data === false) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val() ,'etype': 'conversion' },
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data1) {
		...
		$.each(data, function(index, elem) {
			...
			if(elem.type!=='add'){
				...
				$('#txtItemId').val(data[0]['item_name']);
				$('#hfItemInventoryId').val(data[0]['item_inventory_id']);


				$('#phase_dropdown').select2('val',data[0]['phase_id']);
				

				$('#txtDozenQty').val(data[0]['dozen']);
				$('#txtQty').val( Math.abs(data[0]['s_qty']) );
				$('#txtWeight').val(Math.abs(data[0]['weight']) );
				$('#txtGrWeight').val(data[0]['s_qtyf']);

				$('#txtPRate').val(data[0]['s_rate']);

				$('#txtAmount').val(data[0]['s_amount']);
				$('#txtNetAmount1').val(data[0]['netamount']);


				$('#txtNetCost').val(data[0]['prate']);


				$('#txtOthers').val(data[0]['workdetail']);
			}else{
				appendToTableConsumed('', elem.item_name, elem.item_id,elem.phase_name,elem.phase_id, Math.abs(elem.s_qty), elem.s_rate, elem.s_amount, Math.abs(elem.weight) ,elem.uom,elem.workdetail,elem.item_inventory_id,elem.godown_id,elem.dept_name);				
			}

			// $.each(data1['less'], function(index, elem) {
				
			// 	// appendToTableLess('' ,elem.item_des, elem.item_id,elem.uom, Math.abs(elem.qty),elem.rate,elem.amount);
			// });

			// if(elem.type=='add'){
			// 	appendToTableConsumed('', elem.item_desc_cus, elem.job_id,elem.phase_name,elem.phase_id, elem.s_qty, elem.s_rate, elem.s_amount, elem.weight ,elem.uom_cus,elem.workdetail);
			// }
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'conversion'},
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dzn_qty, uom,others,grweight) {
		...
		"<td class='grweight numeric text-right' data-title='grweight' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTGrWeight text-right'  value='"+ grweight +"'></td>" +
		...
		"<td class='rate numeric text-right' data-title='Rate' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTRate readonly='true' text-right' disabled  value='"+ rate +"'></td>" +
	...
	var calculateNewValues = function ()
	{
		...
		$('.txtTGrWeight').on('keydown', function (e)
		{
			if (e.which == 40) {
				$(this).closest('tr').next().find('input.txtTGrWeight').focus();
				e.preventDefault();
			}
			if (e.which == 38) {
				$(this).closest('tr').prev().find('input.txtTGrWeight').focus();
				e.preventDefault();
			}
		...
		$('.txtTQty,.txtTRate,.txtTDozen,.txtTWeight,.txtTOther,.txtTGrWeight').on('focus', function (e)
		{
			e.preventDefault();
			$(this).select();
		});
	...
	var Table_Total =function(){
		...
		{   
			...
			var grweight = getNumVal($(elem).find('input.txtTGrWeight'));
			...
			totGrWeight =parseFloat(totGrWeight) + parseFloat(grweight);
		...
		$("#txtTotalGrWeight").text(parseFloat(totGrWeight).toFixed(2));
	...
	var validateSingleProductAddConsumed = function() {
		...
		var godown_id = $('#dept_dropdown_cons');
		...
		if ( !godown_id.val() ) {
			$('#dept_dropdown_cons').addClass('inputerror');
			errorFlag = true;
		}
	...
	var appendToTableConsumed = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight, uom,others,inventory_id,godown_id,godown_name) {

		var srno = $('#purchase_table_consumed tbody tr').length + 1;
		var row = 	"<tr>" +
		"<td class='srno numeric' data-title='Sr#' > "+ srno +"</td>" +
		"<td class='item_id' data-title='Id'> "+ item_id +"</td>" +
		"<td class='item_desc' data-title='Description' data-inventory_id='"+ inventory_id +"' data-item_id='"+ item_id +"'> "+ item_desc +"</td>" +
		"<td class='uom' data-title='Uom'> "+ uom +"</td>" +
		"<td class='phase' data-title='Phase' data-phase_id='"+ phase_id +"'> "+ phase +"</td>" +
		"<td class='godown_name' data-title='Warehouse'  data-godown_id='"+ godown_id +"'> "+ godown_name +"</td>" +

		
		"<td class='qty numeric text-right' data-title='Qty' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTQty text-right' disabled=''  value='"+ qty +"'></td>" +
		"<td class='weight numeric text-right' data-title='weight' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTWeight text-right' disabled=''  value='"+ weight +"'></td>" +
	...
	var Table_Total_Consumed =function(){
		...
		var totAmountProduced = getNumVal($('#txtAmount'));
		...
		{   
			...
			if (parseFloat(qty)!=0 && parseFloat(totAmountProduced)!=0 && parseFloat(totalQty)!=0 ){

				amount =  parseFloat(parseFloat(totAmountProduced) * ( parseFloat(qty)*100/parseFloat(totalQty) )/100 ).toFixed(2);
				if (parseFloat(amount)!=0 && parseFloat(qty)!=0 ){
					rate = parseFloat(parseFloat(amount)/parseFloat(qty)).toFixed(2);
				}

			}

			$(elem).find('td.amount').text(amount);
			$(elem).find('input.txtTRate').val(rate);
			totAmount =parseFloat(totAmount) + parseFloat(amount);
	...
	var getSaveObject = function() {
		...
		sd.phase_id = $('#phase_dropdown').val();
		sd.godown_id = $('#dept_dropdown').val();


		sd.dozen = $('#txtDozenQty').val();
		sd.qty = -$('#txtQty').val();
		sd.weight = -$('#txtWeight').val();
		sd.qtyf = $('#txtGrWeight').val();

		sd.rate = $('#txtPRate').val();

		sd.amount = $('#txtAmount').val();
		sd.netamount = $('#txtAmount').val();

		
		sd.prate = $('#txtPRate').val();
		...
		// 	var pledger = {};
		// 	pledger.pledid = '';
		// 	pledger.pid = $('#stitchingid').val();
		// 	pledger.description = prd + ' '+  $('#txtRemarks').val();
		// 	pledger.date = $('#current_date').val();
		// 	pledger.debit = 0;
		// 	pledger.credit = sd.amount;
		// 	pledger.dcno = $('#txtVrnoaHidden').val();
		// 	pledger.invoice = $('#txtVrnoaHidden').val();
		// 	pledger.etype = 'conversion';
		// 	pledger.pid_key = $('#hfItemInventoryId').val();
		// 	pledger.uid = $('#uid').val();
		// 	pledger.company_id = $('#cid').val();
		// 	pledger.isFinal = 0;	
		// 	ledgers.push(pledger);
		...
		$('#purchase_table_consumed').find('tbody tr').each(function( index, elem ) {
			...
			sd.godown_id = $.trim($(elem).find('td.godown_name').data('godown_id'));
			...
			sd.weight = $.trim($(elem).find('input.txtTWeight').val());
			sd.rate = $.trim($(elem).find('input.txtTRate').val());
			sd.prate = $.trim($(elem).find('input.txtTRate').val());
			...
			if(parseFloat(sd.netamount)!=0){
				var pledger = {};
				pledger.pledid = '';
				pledger.pid =$.trim($(elem).find('td.item_desc').data('inventory_id'));
				pledger.description = prd + ' '+  $('#txtRemarks').val();
				pledger.date = $('#current_date').val();
				pledger.credit = 0;
				pledger.debit = sd.netamount;
				pledger.dcno = $('#txtVrnoaHidden').val();
				pledger.invoice = $('#txtVrnoaHidden').val();
				pledger.etype = 'conversion';
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'conversion','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
		...
		bindUI : function() {
			...
			$('input[id="txtItemId"]').autoComplete({
				...
				renderItem: function (item, search)
				{
					...
					'" data-uom_item="'+ item.uom + '" data-item_avg_rate="' + parseFloat(item.item_avg_rate) + '" data-cost_id="' + item.cost_id + '" data-inventory_id="' + item.inventory_id + '" data-item_last_prate="' + parseFloat(item.item_last_prate) + '" data-prate="' + parseFloat(item.cost_price) + '" data-grweight="' + item.grweight + '" data-stqty="' + item.stqty +
				...
				onSelect: function(e, term, item)
				{
					...
					$('#txtPRate').val(item.data('item_avg_rate'));
...
$('input[id="txtItemReceiveId"]').autoComplete({
	...
	renderItem: function (item, search)
	{
		...
		'" data-stweight="' + item.stweight + '" data-item_avg_rate="' + parseFloat(item.item_avg_rate) + '" data-length="' + item.length  + '" data-catid="' + item.catid +
		'" data-subcatid="' + item.subcatid + '" data-desc="' + item.item_des + '" data-short_code="' + item.artcile_no +
		'">' + item.item_des.replace(re, "<b>$1</b>") + '</div>';
	},
	onSelect: function(e, term, item)
	{
		...
		$('#txtConsPRate').val(item.data('item_avg_rate'));

		// fetchLvendor();
		fetchLfiveStocks(itemId);

		$('#dept_dropdown_cons').select2('open');
...
$('#txtLevel3').on('change', function() {
	...
	if (level3 !== "" && level3 !== null) {
			...
			$('#purchase_table').on('click', '.btnRowEdit', function(e) {
				...
				var grweight = $.trim($(this).closest('tr').find('input.txtTGrWeight').val());
			...
			$('#btnAddCons').on('click', function(e) {
				...
				if (!error) {
					...
					var inventory_id = $('#hfItemReceiveInventoryId').val();
					...
					var godown_name = $('#dept_dropdown_cons').find('option:selected').text();
					var godown_id = $('#dept_dropdown_cons').val();
					...
					// var error_stk = Validate_Stock(item_id,edit_qty,qty,edit_weight,weight,godown_name,uom);
					// if(error_stk){


						appendToTableConsumed('', item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,uom,others,inventory_id,godown_id,godown_name);
			...
			$('#purchase_table_consumed').on('click', '.btnRowEdit', function(e) {
				...
				var godown_id = $.trim($(this).closest('tr').find('td.godown_name').data('godown_id'));
				...
				$('#dept_dropdown_cons').select2('val', godown_id);
