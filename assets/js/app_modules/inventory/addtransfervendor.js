var transfervenders = function() {
	...
	var fetchLfiveStocks = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'tr_produce' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id,typ) {
		var crit='';
		if(typ=='pro'){
			var pid = $('#party_dropdownReceive').val();
		}else{
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'tr_produce','crit':crit,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLvendor = function(typ) {
		var crit='';
		if(typ=='pro'){
			var pid = $('#party_dropdownReceive').val();
			var itemid = $('#pro_itemid_dropdown').val();
		}else{
			var pid = $('#party_dropdown11').val();
		...
		$.ajax({
			...
			type : 'POST',
			data : {  'company_id': $('#cid').val(),'etype': 'tr_produce','crit':crit,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
...
var fetchAccount = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'active' : 1,'typee':'tr_produce'},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
					alert('No data found');
...
var populateDataGodowns = function(data) {
	...
	var save = function(purchase) {
		console.log( purchase.stockmain_produce);
		
		$.ajax({
			url : base_url + 'index.php/transfervendor/save',
			type : 'POST',
			data : { 'stockmain' : purchase.stockmain, 'stockdetail' : purchase.stockdetail, 'stockmain_produce' : purchase.stockmain_produce, 'stockdetail_produce' : purchase.stockdetail_produce, 'vrnoa' : purchase.vrnoa ,'voucher_type_hidden':$('#voucher_type_hidden').val(), 'ledger' : purchase.ledger ,'voucher_type_hidden':$('#voucher_type_hidden').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var Account_Voucher = function() {
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			var etype=  'tr_produce';
			var vrnoa = $('#txtVrnoa').val();
			var company_id = $('#cid').val();
			var user = $('#uname').val();

			var hd = ($('#switchHeader').bootstrapSwitch('state') === true) ? '1' : '0';
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/transfervendor/fetch',
			type : 'POST',
	...
	var populateData = function(data) {
		...
		if(data[0]['etype']=='tr_consume'){
			$('#party_dropdown11').select2('val', data[0]['party_id']);
			$('#party_dropdownReceive').select2('val', data[0]['party_id_co']);
		}else{
			$('#party_dropdown11').select2('val', data[0]['party_id_co']);
			$('#party_dropdownReceive').select2('val', data[0]['party_id']);
		}
		
		$('#txtInvNo').val(data[0]['inv_no']);

		$('#txtPartyId').val(data[0]['expense_name']);
		$('#hfPartyId').val(data[0]['expense_id']);
		...
		$.each(data, function(index, elem) {
			if(elem.etype=='tr_consume'){
				...
				appendToTableProduce('', elem.item_name, elem.item_id,elem.phase_name,elem.phase_id,Math.abs(elem.s_qty),elem.s_rate, elem.s_amount, Math.abs(elem.weight),elem.dozen,elem.bag);
				calculateLowerTotal_Produce(Math.abs(elem.s_qty), elem.s_amount, Math.abs(elem.weight),elem.dozen,elem.bag);
			}
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/transfervendor/getMaxVrno',
			type : 'POST',
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/transfervendor/getMaxVrnoa',
			type : 'POST',
			data : {'company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var validateSingleProductAdd_Pro = function() {


		var errorFlag = false;
		var itemEl = $('#pro_item_dropdown');
		var qtyEl = $('#pro_txtQty');
		var rateEl = $('#pro_txtPRate');
		var phase = $('#pro_phase_dropdown');
	...
	var appendToTable = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dzn_qty,bag) {
		...
		var row = 	"<tr>" +
						"<td class='srno numeric' data-title='Sr#' > "+ srno +"</td>" +
				 		"<td class='item_id' data-title='Id' data-item_id='"+ item_id +"'> "+ item_id +"</td>" +
	...
	var appendToTableProduce = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dzn_qty,bag) {

		var srno = $('#pro_purchase_table tbody tr').length + 1;
		var row = 	"<tr>" +
						"<td class='srno numeric' data-title='Sr#' > "+ srno +"</td>" +
						"<td class='item_id' data-title='Id' data-item_id='"+ item_id +"'> "+ item_id +"</td>" +
				 		"<td class='item_desc' data-title='Description' data-item_id='"+ item_id +"'> "+ item_desc +"</td>" +
				 		...
				 		"<td class='qty numeric' data-title='Qty' style='text-align:right;'>  "+ qty +"</td>" +
					 	"<td class='weight numeric' data-title='Weigh' style='text-align:right;'> "+ weight +"</td>" +
					 	"<td class='rate numeric hide' data-title='Rate' style='text-align:right;'> "+ rate +"</td>" +
					 	"<td class='amount numeric hide' data-title='Amount' style='text-align:right;'> "+ amount +"</td>" +
					 	"<td><a href='' class='btn btn-primary btnRowEdit'><span class='fa fa-edit'></span></a> <a href='' class='btn btn-primary btnRowRemove'><span class='fa fa-trash-o'></span></a> </td>" +
				 	"</tr>";
		$(row).appendTo('#pro_purchase_table');
	}
	...
	var getSaveObject = function() {
		...
		var ledgers = [];
		var stockmain = {};
		var stockmain_produce = {};
		var stockdetail = [];
		var stockdetail_produce = [];
		...
		stockmain.party_id_co = $('#party_dropdownReceive').val();
		...
		stockmain.vehicle_id = $('#hfPartyId').val();


		stockmain.uid = $('#uid').val();
		stockmain.company_id = $('#cid').val();//$('#cid').val();

		stockmain_produce.vrno = $('#txtVrnoHidden').val();
		stockmain_produce.vrnoa = $('#txtVrnoaHidden').val();
		stockmain_produce.vrdate = $('#current_date').val();
		stockmain_produce.party_id_co = $('#party_dropdown11').val();
		stockmain_produce.party_id = $('#party_dropdownReceive').val();
		stockmain_produce.inv_no = $('#txtInvNo').val();
		stockmain_produce.bilty_date = $('#due_date').val();
		stockmain_produce.received_by = $('#receivers_list').val();
		stockmain_produce.transporter_id = $('#transporter_dropdown').val();
		stockmain_produce.remarks = $('#txtRemarks').val();
		stockmain_produce.etype = 'tr_produce';
		stockmain_produce.namount = $('#txtNetAmount').val();
		stockmain_produce.workorder = $('#txtOrderNo').val();
		stockmain_produce.discp = $('#txtDiscount').val();
		stockmain_produce.discount = $('#txtDiscAmount').val();
		stockmain_produce.expense =$('#txtExpAmount').val();
		stockmain_produce.exppercent = $('#txtExpense').val();
		stockmain_produce.tax = $('#txtTaxAmount').val();
		stockmain_produce.taxpercent = $('#txtTax').val();
		stockmain_produce.paid = $('#txtPaid').val();
		stockmain_produce.uid = $('#uid').val();
		stockmain_produce.company_id = $('#cid').val();//$('#cid').val();
		...
		var prd='';
		$('#pro_purchase_table').find('tbody tr').each(function( index, elem ) {
			var sd = {};
			sd.stid = '';
			sd.item_id = $.trim($(elem).find('td.item_desc').data('item_id'));
			sd.phase_id = $.trim($(elem).find('td.phase').data('phase_id'));
		...
		pledger.etype = 'tr_produce';
		pledger.pid_key = $('#stitchingid').val();
		pledger.uid = $('#uid').val();
		...
		data.stockmain_produce = stockmain_produce;
		
		data.stockdetail = stockdetail;
		data.stockdetail_produce = stockdetail_produce;
		data.ledger = ledgers;
	...
	var validateSave = function() {
		...
		var partyEl_Receive = $('#party_dropdownReceive');

		var expense_id = $('#hfPartyId');


		var deptEl = $('#party_dropdownReceive');
		...
		if ( !partyEl_Receive.val() ) {
			partyEl_Receive.addClass('inputerror');
			errorFlag = true;
		}
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/transfervendor/delete',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'tr_produce','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var calculateLowerTotal = function(qty, amount, weight,dozen,bag) {
		...
		$('#txtNetAmount').val(net.toFixed(2));
	}

	var calculateLowerTotal_Produce = function(qty, amount, weight,dozen,bag) {

		
		var _dozen = getNumVals($('#pro_txtTotalDozen'));
		var _bag = getNumVals($('#pro_txtTotalBag'));
		var _qty = getNumVals($('#pro_txtTotalQty'));
		var _weight = getNumVals($('#pro_txtTotalWeight'));
		var _amnt = getNumVals($('#pro_txtTotalAmount'));

		


		var tempQty = parseFloat(_qty) + parseFloat(qty);
		$('#pro_txtTotalQty').text(tempQty);
		var tempDozen = parseFloat(_dozen) + parseFloat(dozen);
		$('#pro_txtTotalDozen').text(tempDozen);
		var tempBag = parseFloat(_bag) + parseFloat(bag);
		$('#pro_txtTotalBag').text(tempBag);
		var tempAmnt = parseFloat(_amnt) + parseFloat(amount);
		$('#pro_txtTotalAmount').text(tempAmnt);

		var totalWeight = parseFloat(parseFloat(_weight) + parseFloat(weight)).toFixed(2);
		$('#pro_txtTotalWeight').text(totalWeight);
		// $('#txtNetAmount').val(parseFloat(tempAmnt).toFixed(2));
	}
...
var calculateUpperSum_Produce = function() {

		var _dozen=getNumVal($('#pro_txtDozenQty'));
		var _qty = getNumVal($('#pro_txtQty'));
		var _amnt = getNumVal($('#pro_txtAmount'));
		var _net = getNumVal($('#pro_txtNet'));
		var _prate = getNumVal($('#pro_txtPRate'));
		var _gw = getNumVal($('#pro_txtGWeight'));
		var _weight=getNumVal($('#pro_txtWeight'));
		var _uom=$('#pro_txtUom').val().toLowerCase();
		// var _dozen=getNumVal($('#pro_txtdozen'));

		// var _uom=$('#pro_txtUom').val().toUpperCase();
		// alert('uom_item ' + _uom);
		if(_uom === 'dozen' ){
		    var _tempAmnt = parseFloat(_dozen) * parseFloat(_prate);  
		} else {
		...
		var _tempWeight =parseFloat(_gw) * parseFloat(_qty);
		
		$('#pro_txtWeight').val(_tempWeight.toFixed(2));
		$('#pro_txtAmount').val(parseFloat(_tempAmnt).toFixed(2));
	}
	...
	var resetFields = function() {
		...
		$('#pro_txtTotalAmount').text('');
		$('#pro_txtTotalDozen').text('');
		$('#pro_txtTotalBag').text('');
		$('#pro_txtTotalQty').text('');
		$('#pro_txtTotalWeight').text('');
	...
	return {
		...
		bindUI : function() {
			...
			// $('#pro_phase_dropdown,#pro_item_dropdown,#pro_itemid_dropdown,#phase_dropdown,#item_dropdown,#itemid_dropdown,#pType_dropdown,#party_dropdown11,#dept_dropdown,#transporter_dropdown').select2();
			// $('#pro_item_dropdown').select2({dropdownAutoWidth : false});
			// $('#item_dropdown').select2({dropdownAutoWidth : true});
			// $('#item_dropdown').css('width', '282px');
			...
			$('#pro_txtWeight').on('keypress', function(e) {
				if (e.keyCode === 13) {
					e.preventDefault();
					$('#pro_btnAdd').trigger('click');
				}
			});
			...
			$('#pro_itemid_dropdown').on('change', function() {
				var item_id = $(this).val();
				var prate = $(this).find('option:selected').data('prate');
				var grweight = $(this).find('option:selected').data('grweight');
				var uom_item = $(this).find('option:selected').data('uom_item');
				...
				// var stqty = $(this).find('option:selected').data('stqty');
				// var stweight = $(this).find('option:selected').data('stweight');
				// $('#pro_stqty_lbl').text('Item,     Qty:' + stqty + ', Weight ' + stweight);

				// $('#pro_txtPRate').val(parseFloat(prate).toFixed(2));
				$('#pro_item_dropdown').select2('val', item_id);
				$('#pro_txtGWeight').val(parseFloat(grweight).toFixed());
				$('#pro_txtUom').val(uom_item);

				fetchLfiveStocks(item_id,'pro');
				fetchLfiveRates(item_id,'pro');
				fetchLvendor('pro');
				$('#pro_phase_dropdown').select2('open');
				// calculateUpperSum();
				// $('#pro_txtQty').focus();
			});
			
			$('#pro_item_dropdown').on('change', function() {
				var item_id = $(this).val();
				var prate = $(this).find('option:selected').data('prate');
				var grweight = $(this).find('option:selected').data('grweight');
				var uom_item = $(this).find('option:selected').data('uom_item');
				...
				// $('#pro_txtPRate').val(parseFloat(prate).toFixed(2));
				$('#pro_itemid_dropdown').select2('val', item_id);
				$('#pro_txtGWeight').val(parseFloat(grweight).toFixed(2));
				$('#pro_txtUom').val(uom_item);
			...
			$('#txtQty').on('input', function() {
				...
				var _gw = getNumVal($('#txtGWeight'));
				var _tempWeight =parseFloat(_gw) * parseFloat(_qty);
				$('#txtWeight').val(_tempWeight.toFixed(2));
			...
			// $('#pro_txtQty').on('input', function() {
			// 	if (parseFloat($(this).val()) !=0){
			// 		var q = parseInt(parseFloat($(this).val())/12);
			// 	}else{
			// 		var q = 0;
			// 	}
			// 	$('#pro_txtDozenQty').val(q);
			// 	calculateUpperSum_Produce();
			// });
			// $('#pro_txtDozenQty').on('input', function() {
			...
			$('#pro_txtPRate').on('input', function() {
				calculateUpperSum_Produce();
			});
			...
			$('#pro_btnAdd').on('click', function(e) {
				e.preventDefault();

				var error = validateSingleProductAdd_Pro();
				if (!error) {

					var item_desc = $('#pro_item_dropdown').find('option:selected').text();
					var item_id = $('#pro_item_dropdown').val();
					var dozen = $('#pro_txtDozenQty').val();
					var bag = $('#pro_txtBag').val();
					var qty = $('#pro_txtQty').val();
					var rate = $('#pro_txtPRate').val();
					var weight = $('#pro_txtWeight').val();
					var amount = $('#pro_txtAmount').val();
					var phase = $('#pro_phase_dropdown').find('option:selected').text();
					var phase_id = $('#pro_phase_dropdown').val();

					// reset the values of the annoying fields
					$('#pro_itemid_dropdown').select2('val', '');
					$('#pro_item_dropdown').select2('val', '');
					$('#pro_txtQty').val('');
					$('#pro_txtDozenQty').val('');
					$('#pro_txtBag').val('');
					$('#pro_txtPRate').val('');
					...
					$('#pro_txtAmount').val('');
					$('#pro_txtGWeight').val('');
					$('#pro_phase_dropdown').select2('val','');


					appendToTableProduce('', item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dozen,bag);
					calculateLowerTotal_Produce(qty, amount, weight,dozen,bag);
					$('#pro_stqty_lbl').text('Produce Item');
					$('#pro_item_dropdown').focus();
					$('#pro_item_dropdown').select2('open');
				} else {
					alert('Correct the errors!');
				}
			...
			$('#pro_purchase_table').on('click', '.btnRowRemove', function(e) {
				e.preventDefault();
				var qty = $.trim($(this).closest('tr').find('td.qty').text());
				var dozen = $.trim($(this).closest('tr').find('td.dzn_qty').text());
				var bag = $.trim($(this).closest('tr').find('td.bag').text());
			...
			$('#pro_purchase_table').on('click', '.btnRowEdit', function(e) {
				e.preventDefault();
				...
				$('#pro_itemid_dropdown').select2('val', item_id);
				$('#pro_item_dropdown').select2('val', item_id);
				$('#pro_item_dropdown').trigger('change');

				var grweight = $('#pro_item_dropdown').find('option:selected').data('grweight');

				$('#pro_txtGWeight').val(parseFloat(grweight).toFixed());
				$('#pro_txtQty').val(qty);
				$('#pro_txtDozenQty').val(dozen);
				$('#pro_txtBag').val(bag);
				$('#pro_txtPRate').val(rate);
				$('#pro_txtWeight').val(weight);
				$('#pro_txtAmount').val(amount);
				$('#pro_phase_dropdown').select2('val', phase_id);
				calculateLowerTotal_Produce("-"+qty, "-"+amount, '-'+weight , '-'+dozen,'-'+bag);
				// now we have get all the value of the row that is being deleted. so remove that cruel row
				$(this).closest('tr').remove();	// yahoo removed
			});
		...
		initSave : function() {
			...
			if (!error) {
				...
				var rowsCount1 = $('#pro_purchase_table').find('tbody tr').length;

				if (rowsCount + rowsCount1 > 0 ) {
					save(saveObj);
				} else {
