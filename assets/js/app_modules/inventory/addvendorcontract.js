...
var populateDataGodowns = function(data) {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			appendToTable('', elem.item_name, elem.item_id,elem.phase_name,elem.phase_id,Math.abs(elem.qty),elem.rate, elem.amount, Math.abs(elem.weight),elem.dozen,elem.bag,elem.frate,elem.lprate,elem.qtyf,elem.bundle);
	...
	var getSaveObject = function() {
		...
		stockmain.etype = 'venderscontract';
...
var Validate_Stock = function(item_id,edit_qty,qty_chk,edit_weight,weight_chk,godown,uom) {
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'venderscontract','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					// if(error_stk){
						...
						// var appendToTable = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dzn_qty,bag,frate,lprate,qtyf,bundle) {
						appendToTable('', item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dozen,bag,req_weight,bag_rate,wastage,bag_weight);
