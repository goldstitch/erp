var Issuetovender = function() {
	...
	var fetchLfiveStocks = function(item_id) {
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'stitchingcontract' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'stitchingcontract','crit':crit,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLvendor = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {  'company_id': $('#cid').val(),'etype': 'stitchingcontract','crit':crit,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
...
var populateDataGodowns = function(data) {
	...
	var save = function(purchase) {
		...
		$.ajax({
			url : base_url + 'index.php/vendorcontract/save',
			type : 'POST',
			data : { 'etype' : 'stitchingcontract','stockmain' : purchase.stockmain, 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa,'voucher_type_hidden':$('#voucher_type_hidden').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/vendorcontract/fetch',
			type : 'POST',
			data : { 'etype' : 'stitchingcontract','vrnoa' : vrnoa , 'company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			appendToTable('', elem.item_name, elem.item_id,elem.phase_name,elem.phase_id,Math.abs(elem.qty),elem.rate, elem.amount, Math.abs(elem.weight),elem.dozen,elem.bag,elem.frate,elem.lprate,elem.qtyf,elem.bundle,elem.phaseTo,elem.phase_id2,elem.uom);
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/vendorcontract/getMaxVrno',
			type : 'POST',
			data : {'etype' : 'stitchingcontract','company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/vendorcontract/getMaxVrnoa',
			type : 'POST',
			data : {'etype' : 'stitchingcontract','company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dzn_qty,bag,frate,lprate,qtyf,bundle,phaseTo,phase_id_to,uom) {

		var srno = $('#purchase_table tbody tr').length + 1;
		var row = 	"<tr>" +
						"<td class='srno numeric' data-title='Sr#' > "+ srno +"</td>" +
				 		"<td class='idd' data-title='Id'> "+ item_id +"</td>" +
				 		...
				 		"<td class='phaseTo' data-title='PhaseTo' data-phase_id2='"+ phase_id_to +"'> "+ phaseTo +"</td>" +
	...
	var getSaveObject = function() {
		...
		stockmain.etype = 'stitchingcontract';
		...
		$('#purchase_table').find('tbody tr').each(function( index, elem ) {
			...
			sd.phase_id2 = $.trim($(elem).find('td.phaseTo').data('phase_id2'));
...
var Validate_Stock = function(item_id,edit_qty,qty_chk,edit_weight,weight_chk,godown,uom) {
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/vendorcontract/delete',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'stitchingcontract','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var calculateUpperSum = function() {
		...
		$('#txtWeight').val(parseFloat(_weight + _wastage_add).toFixed(2));
		...
		bindUI : function() {
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					var phaseTo = $('#phase_dropdown_To').find('option:selected').text();
					var phase_id = $('#phase_dropdown').val();
					var phase_id_To = $('#phase_dropdown_To').val();
					...
					// if(error_stk){
						...
						// var appendToTable = function(srno, item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dzn_qty,bag,frate,lprate,qtyf,bundle) {
						appendToTable('', item_desc, item_id,phase,phase_id ,qty, rate, amount, weight,dozen,bag,req_weight,bag_rate,wastage,bag_weight,phaseTo,phase_id_To,uom);
			...
			$('#purchase_table').on('click', '.btnRowEdit', function(e) {
				...
				var phase_id_to = $.trim($(this).closest('tr').find('td.phaseTo').data('phase_id2'));
				...
				$('#phase_dropdown_To').select2('val', phase_id_to);
				calculateLowerTotal("-"+qty, "-"+amount, '-'+weight , '-'+dozen,'-'+bag);
