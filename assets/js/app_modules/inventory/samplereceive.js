...
var Transfer = function() {
	...
	var sample_issue_material = function(id,code) {
		
		$.ajax({
			url : base_url + 'index.php/saleorder/sample_issue_material',
			type : 'POST',
			data : { 'id' : id },
			dataType : 'JSON',
			success : function(data) {
	...
	var fetch_sample = function(id,code) {
		...
		$.ajax({
			...
			dataType : 'JSON',
			success : function(data) {
				$('.Lstocks_table3').find('tbody tr').remove();
				if (data === 'false') 
				{
					alert('No data found');
				} 
	...
	var fetchReports = function (from, to,companyid,etype,uid) {
		...
		if (typeof dTable != 'undefined') {
		    ...
		    var save = function(stocknavigation) {
		    	...
		    	$.ajax({
		    		url : base_url + 'index.php/sampletransfer/save_receive',
		    		type : 'POST',
	...
	var post_chk = function(vrnoa) {
		$.ajax({
			...
			var populate2 = function(data) {
				...
				$.each(data, function(index, elem) {
					materialtable2(elem.item_id,elem.description,elem.qty);
				});
		    }
		    ...
		    var fetch = function(vrnoa) {
		    	...
		    	$.ajax({
		    		...
		    		url : base_url + 'index.php/sampletransfer/fetch_receive',
		    		type : 'POST',
			...
			var fetch_consume = function(vrnoa) {

		    	$.ajax({
		    		url : base_url + 'index.php/sampletransfer/fetch_consume',
		    		type : 'POST',
		    		data : { 'vrnoa' : vrnoa, 'company_id': $('#cid').val() },
		    		dataType : 'JSON',
		    		success : function(data) {
						$('#purchase_table2').find('tbody tr').remove();
		    			
		    			populate_detail(data);
			...
			var fetch_consumes = function(vrnoa) {

		    	$.ajax({
		    		url : base_url + 'index.php/sampletransfer/fetch_consumes',
		    		type : 'POST',
		    		data : { 'vrnoa' : vrnoa, 'company_id': $('#cid').val() },
		    		dataType : 'JSON',
		    		success : function(data) {
		    ...
		    var populateData = function(data) {
		    	...
		    	$.each(data, function(index, elem) {
		    		appendToTable('1', elem.item_name, elem.item_id, '', elem.godown_id2, elem.uom, elem.qty, elem.weight, elem.dept_to, elem.godown_id,elem.dozen);
		  
		    	});

				sample_issue_material(data[0]['job_id']);
		
		    }

			var populate_detail = function(data) {
		   
		    	$.each(data, function(index, elem) {
		    		appendToTable2('1', elem.item_name, elem.item_id,elem.dept_to, elem.godown_id2, elem.uom, -elem.qty, elem.weight, '', elem.godown_id,elem.dozen);
		 
		    	});
		    }

			var populate_details = function(data) {
		   
		    	$.each(data, function(index, elem) {
		    		appendToTable3('1', elem.item_name, elem.item_id,elem.dept_to, elem.godown_id2, elem.uom, -elem.qty, elem.weight, '', elem.godown_id,elem.dozen);
		    ...
		    var getMaxVrno = function() {
		    	...
		    	$.ajax({
		    		...
		    		url : base_url + 'index.php/sampletransfer/getMaxVrno_receive',
		    		type : 'POST',
		    ...
		    var getMaxVrnoa = function() {
		    	...
		    	$.ajax({
		    		...
		    		url : base_url + 'index.php/sampletransfer/getMaxVrnoa_receive',
		    		type : 'POST',
		    		data : {'company_id':$('#cid').val()},
		    		dataType : 'JSON',
		    		success : function(data) {
			...
			var appendToTable2 = function(srno, item_desc, item_id, deptfrom, deptfrom_id, uom, qty, weight, deptto, deptto_id, dozen) {
		    	...
		    	"<td class='weight' data-title='Weight' style='text-align:right;'> "+ weight +"</td>" +
		    	"<td class='deptto' data-deptto_id='"+ deptto_id +"' data-title='To'> "+ deptto +"</td>" +
		    	"<td data-title='Action'> <a class='btn btn-primary btnRowRemove2'><span class='fa fa-trash-o'></span></a> </td>" +
		    	"</tr>";
			...
			var appendToTable3 = function(srno, item_desc, item_id, deptfrom, deptfrom_id, uom, qty, weight, deptto, deptto_id, dozen) {
		    	...
		    	"<td class='uom' data-title='UOM'> "+ uom +"</td>" +
		    	"<td class='qty' data-title='Qty' style='text-align:right;'> "+ qty +"</td>" +
		    	"<td class='weight' data-title='wo#' style='text-align: right; max-width:30px;'>  <input type='number' class='form-control text-left txtTwo' value='"+ weight +"'></td>" +
		    	"<td class='deptto' data-deptto_id='"+ deptto_id +"' data-title='To'> "+ deptto +"</td>" +
		    	"<td data-title='Action'><a class='btn btn-primary btnRowRemove3'><span class='fa fa-trash-o'></span></a></td>" +
		    	"</tr>";
		    ...
		    var getSaveObject = function() {
		    	...
		    	stockmain.etype = 'sample_receive';
			...
			var getSaveObject2 = function() {
				...
				od.godown_id = $.trim($(elem).find('td.deptfrom').data('deptfrom_id'));
				od.godown_id2 = $.trim($(elem).find('td.deptto').data('deptto_id'));
				od.qty = "-" + $.trim($(this).closest('tr').find('input.txtTwo').val());
				od.dozen = $.trim($(elem).find('td.dozen').text());
				od.weight = $.trim($(elem).find('td.qty').text());
				od.uom = $.trim($(elem).find('td.uom').text());
				od.issue_id = $('#issue_id').val();
				od.receive_id =$('#emp_id').val();
				od.job_id =$('#job_id').val();
	...
	var materialtable = function(item_id,material,qty) {
		...
		var row = 	"<tr>" +
		"<td class='1' data-title='Description' data-item_id='"+ qty +"'> "+ material +"</td>" +
	...
	var materialtable2 = function(item_id,material,qty) {

		var srno = $('.Lstocks_table3 tbody tr').length + 1;
		var row = 	"<tr>" +
		"<td class='1' data-title='Description' data-item_id='"+ item_id +"'> "+ material +"</td>" +
		"<td class='text-right 2' data-title='Description' data-qty='"+ qty +"'> "+ qty +"</td>" +

		"</tr>";
		$(row).appendTo('.Lstocks_table3');
	}
		...
		bindUI : function() {
			...
			//	checkqty(item_id,deptfrom_id);
				...
				if (!error) {
					...
					var uom = $('#txtUOM').val();
					var qty = $('#txtSQty').val();
					var dozen = $('#txtRec').val();
					var weight = $('#txtWeight').val();
					var deptto = $('#deptto_dropdown').val();
					var item_id = $('#hfItemId').val(); 
					var deptfrom = $('#deptfrom_dropdown').val();

			        appendToTable('1', item_desc, item_id, '', '', uom, qty, weight, deptto, todept_id, dozen);

					$('.Lstocks_table3').find('tbody tr').each(function( index, elem ) {
	
						var material =$.trim($(this).closest('tr').find('td.1').text());
						var m_qty =$.trim($(this).closest('tr').find('td.2').text());
						var item_id_ =$.trim($(elem).find('td.1').data('item_id'))
			
						appendToTable2('1', material, item_id_, deptfrom, fromdept_id, uom, parseFloat(m_qty*qty).toFixed(2), weight, '', '', dozen)
					...
					var items=[];
					var i =0;
					$('.Lstocks_table2').find('tbody tr').each(function( index, elem ) {

						items[i]= r_qty=$.trim($(elem).find('td.1').data('item_id'));
						i=i+1;
					});
					...
					$('.Lstocks_table3').find('tbody tr').each(function( index, elem ) {
						...
						var material =$.trim($(this).closest('tr').find('td.1').text());
						var qty_=$.trim($(this).closest('tr').find('td.2').text());
						var item_id_ =$.trim($(elem).find('td.1').data('item_id'));
						var final_qty=parseFloat(items[j])-parseFloat(qty_);
						j=j+1;
						
						appendToTable3('1', material, item_id_, deptfrom, fromdept_id, uom, parseFloat(final_qty).toFixed(2), parseFloat(final_qty).toFixed(2), '', '', dozen)
   ...
   //          }
	...
	$('#sample_id').on('input', function() {
		var job= $('#sample_id').val();
		fetch_sample(job);
	});
