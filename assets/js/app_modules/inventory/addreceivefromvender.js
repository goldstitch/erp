var Receivefromvender = function() {
	...
	var fetchLfiveStocks = function(item_id) {
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'rfv' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'rfv','crit':crit},
			dataType : 'JSON',
			success : function(data) {
	...
	var save = function(purchase) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'stockmain' : purchase.stockmain , 'vendordetail' : JSON.stringify(purchase.vendordetail), 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa, 'voucher_type_hidden':$('#voucher_type_hidden').val(), 'ledger' : purchase.ledger ,'etype':'rfv' },
			dataType : 'JSON',
			success : function(data) {
	...
	var CheckDuplicateVoucher = function(stockmain) {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'gp' : $('#txtBilty').val(), 'vrnoa' : $('#txtVrnoaHidden').val(),'etype':'rfv', 'company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
				console.log(data);
				if (data === false) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val() ,'etype': 'rfv' },
			dataType : 'JSON',
			success : function(data) {
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'rfv'},
			dataType : 'JSON',
			success : function(data) {
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'rfv','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var calculateUpperSum = function() {
		...
		var wastage_perc = 0;
		var consumed_qty = getNumVals($('#txtConsTotalQty'));


		if(parseFloat(consumed_qty)!=0 && parseFloat(_qty) ){
			wastage_perc = parseFloat( (parseFloat(consumed_qty) - parseFloat(_qty)) * 100 / parseFloat(consumed_qty)).toFixed(2);
		}

		$('#txtWastagePercent').val(wastage_perc);
...
var getcritBulk = function (etype){
		...
		// var catid=$('#hfItemCatId').select2("val");
	...
	var populateDataIrm = function(data) {
		...
		$.each(data, function(index, elem) {
			if (elem.detype === 'fabric' ) {
				...
				qty = parseFloat(parseFloat(elem.s_qtyf) * parseFloat(pro_qty)).toFixed(2) ;
				...
				appendToTableConsumed('', elem.item_name, elem.item_id,'','', qty,parseFloat(elem.item_avg_rate).toFixed(0),amount, weight,elem.uom,others);
				// appendToTableConsumed = function(srno, item_desc, item_id,phase,phase_id ,								qty, rate, amount, weight, uom,others)
...
$('input[id="txtItemReceiveId"]').autoComplete({
	...
	renderItem: function (item, search)
	{
		...
		return '<div class="autocomplete-suggestion ' + selected + '" data-val="' + search + '" data-photo="' + item.photo + '" data-item_id="' + item.item_id + '" data-size="' + item.pack + '" data-bid="' + item.bid +
		'" data-uom_item="'+ item.uom + '" data-cost_id="' + item.cost_id + '" data-inventory_id="' + item.inventory_id + '" data-item_last_prate="' + parseFloat(item.item_avg_rate) + '" data-prate="' + parseFloat(item.cost_price) + '" data-grweight="' + item.grweight + '" data-stqty="' + item.stqty +
		'" data-stweight="' + item.stweight + '" data-length="' + item.length  + '" data-catid="' + item.catid +
		'" data-subcatid="' + item.subcatid + '" data-desc="' + item.item_des + '" data-short_code="' + item.artcile_no +
		'">' + item.item_des.replace(re, "<b>$1</b>") + '</div>';
	},
