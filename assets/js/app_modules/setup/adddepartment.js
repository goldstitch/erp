var Shift = function() {
	...
	var fetch = function(did) {

		$.ajax({
			url : base_url + 'index.php/department/fetchDepartment',
			type : 'POST',
			data : { 'did' : did },
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem){
			...
			$('#txtId').val(elem.did);
			$('#txtIdHidden').val(elem.did);
			$('#txtName').val(elem.name);
			$('#txtStrength').val(elem.strength);
	...
	var getSaveObject = function() {
		...
		var obj = {};
		obj.did = $.trim($('#txtIdHidden').val());
		obj.name = $.trim($('#txtName').val());
		obj.strength = $.trim($('#txtStrength').val());
	...
	return {
		...
		bindUI : function() {
			...
			$('.btn-edit-dept').on('click', function(e) {
				...
				fetch($(this).data('did'));		// get the class detail by id
