var emb_total_=0,fab_total=0,dye_total=0,price=0,t_cost=0,tcost=0,pcost=0,total_=0,sum =0,total1=0,dig_total_=0;
var AddItem = function() {
	...
	var save = function(obj) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/saleorder/save_sample_production',
			type : 'POST',
			data : { 'production' :obj.sample_production,'embroidory' : obj.embroidory ,'fabricdye' : obj.fabric_dye,'cutstitch' : obj.cut_stitch,'stitchaccesseries' : obj.stitch_accesseries,'embelishment' : obj.embelishment,'presspack' : obj.press_pack,'summary' : obj.summary,'material':obj.material,'digital':obj.digital_printing},
			dataType : 'JSON',
...
var deleteVoucher = function(id,code) {
	...
	$.ajax({
		url : base_url + 'index.php/saleorder/delete_sample_production',
		type : 'POST',
...
var getsaveproduct = function() {
	...
	$('#purchase_table2').find('tbody tr').each(function( index, elem ) {
		...
		$('#purchase_table5').find('tbody tr').each(function( index, elem ) {
			...
			embell.kaviya_cost =$.trim($(this).closest('tr').find('td.8').text());
			embell.material_cost =$('#esttime').val();
			embell.sub_total =$('#subtotal_').val();
			embelishment.push(embell);
		...
		digital_printing.push(dig);
	
		});

   

	var  sample_production =[];
	var pro = {};
	pro.id =$('#txtVrnoa').val();
	...
	var data = {};
	data.embroidory = embroidory;
	data.sample_production = sample_production;
	data.material= material;
	data.fabric_dye = fabric_dye;
	data.cut_stitch = cut_stitch;
	data.stitch_accesseries = stitch_accesseries;
	data.embelishment = embelishment;
	data.press_pack = press_pack;
	data.summary = summary;
	data.digital_printing = digital_printing;
...
var populate6 = function(data) {
	...
	var getMaxId = function() {
		...
		$.ajax({
			url : base_url + 'index.php/saleorder/getmaxid',
			type : 'POST',
			dataType : 'JSON',
			success : function(data) {
				$('#txtVrnoa').val(data);
	...
	$('.btnAdd6').on('click', function(e) {
		...
		appendToTable6(type,p_name,p_rate,p_qty,p_cost);
...
$('#c_disc').on('change', function() {
	...
	$('#c_code').val(c_code);
			...
			$('#txtVrnoa').on('change', function() {
				...
				fetch_press($(this).val(),$('#code').val());
				fetch_emb($(this).val(),$('#code').val());
				fetch_embl($(this).val(),$('#code').val());
				fetch_cut($(this).val(),$('#code').val());
				fetch_fab($(this).val(),$('#code').val());
				fetch_stitch($(this).val(),$('#code').val());
				fetch_digital($(this).val(),$('#code').val());
				fetch_pack_material($(this).val(),$('#code').val());
				fetch_embell_material($(this).val(),$('#code').val());
