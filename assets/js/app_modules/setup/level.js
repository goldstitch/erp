var Level = function() {
	...
	var save = function( l1Obj, level ) {
		...
		$.ajax({
			...
			dataType : 'JSON',
			data : { 'levelDetail' : l1Obj, 'level' : level,'uid':$('#uid').val() },
			type : 'POST',
	...
	var populateDataL1 = function(data) {
		...
		$.each(data, function(index, elem){
			...
			$(settings.txtBsLevel).val(elem.bslevel);
	...
	var populateDataL2 = function(data) {
		...
		$.each(data, function(index, elem){
			...
			$(settings.level1_dropdown).val(elem.l1);
	...
	var populateDataL3 = function(data) {
		...
		$.each(data, function(index, elem){
			...
			$(settings.level2_dropdown).val(elem.l2);
	...
	var validateL1Save = function() {
		...
		var bslevel = $.trim($(settings.txtBsLevel).val());
		...
		if ( bslevel === "" ) {
			$(settings.txtBsLevel).addClass('inputerror');
			errorFlag = true;
		}
	...
	var getSaveL1Obj = function() {
		...
		return obj = {
			...
			bslevel : $.trim($(settings.txtBsLevel).val()),
			uid : $(settings.uid).val()
		};
	}
	...
	return {
		...
		bindUI : function() {
			...
			$(settings.btnEditLevel1).on('click', function(e) {
				e.preventDefault();		// prevent the default behaviour of the link
			...
			$(settings.btnEditLevel2).on('click', function(e) {
				e.preventDefault();		// prevent the default behaviour of the link
			...
			$(settings.btnEditLevel3).on('click', function(e) {
				e.preventDefault();		// prevent the default behaviour of the link
