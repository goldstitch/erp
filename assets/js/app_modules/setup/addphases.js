var Category = function() {
	...
	var save = function( phase ) {

		$.ajax({
			url : base_url + 'index.php/phase/savePhase',
			type : 'POST',
			data : { 'phase' : phase },
			dataType : 'JSON',
			...
			success : function(data) {
				...
				if (data == "duplicate") {
					alert('Phase name already saved!');
				} else {					
					if (data.error === 'false') {
	...
	var fetch = function(id) {
		...
		$.ajax({
			url : base_url + 'index.php/phase/fetchPhase',
			type : 'POST',
			data : { 'id' : id },
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(elem) {
		...
		// $('#phaseType_dropdown').select2('val',elem.ptypeid);
	}
	...
	var getMaxId = function() {
		...
		$.ajax({
			url : base_url + 'index.php/phase/getMaxPhaseId',
			type : 'POST',
			dataType : 'JSON',
			success : function(data) {
	...
	var validateSave = function() {
		...
		var phasetype = $('#phaseType_dropdown').val();
		...
		if ( phasetype === '' ) {
			$('#phaseType_dropdown').addClass('inputerror');
			errorFlag = true;
		}
	...
	var getSaveObject = function() {
		...
		// obj.ptypeid = $('#phaseType_dropdown').val();
		return obj;
	}
