var YarnIssue = function() {
	...
	var save = function( yarn_issue ) {
		...
		$.ajax({
			...
			data : {'etype':'frv', 'stockmain' : yarn_issue.stockmain, 'stockdetail' : yarn_issue.stockdetail, 'vrnoa' : yarn_issue.vrnoa,'voucher_type_hidden':$('#vouchertypehidden').val() , 'ledger' : JSON.stringify(yarn_issue.ledger)},
	...
	var populateDataContract = function(elem) {
		...
		$('#txtTotalMeter').val(elem.fabric_balqty);
		$('#txtChPick').val(elem.conversionchargespick); 
		$('#txtChMtr').val(elem.conversionchargesmeter); 



		$('#txtContractTotalBag').val(elem.bagtotal);
		$('#txtContractWrapBag').val(elem.bagwarp);
		$('#txtContractWeftBag').val(elem.bagwept);


		$('#contqty').val(elem.qty);
		$('#txtReceiveMeter').val(elem.fabric_rcvqty);
		$('#balanceqty').val(elem.fabric_balqty);

		$('#txtYarnRate').val(elem.yarn_bag_avg_rate);
	...
	var fetchContractEdit = function(contract_id) {
	...
	var populateDataContractEdit = function(elem) {
	...
	var populateData = function(data) {
		...
		$('#txtSaleTax').val(elem.taxpercent);
		$('#txtSaleTaxValue').val(elem.tax);
		$('#txtNetAmount').val(elem.namount);


		$('#txtIncomeTax').val(elem.exppercent);
		$('#txtIncomeTaxValue').val(elem.expense);
		...
		$.each(data, function(index, elem) {
			if(elem.type=='finish'){
				...
				$('#txtFreshFabric').val(elem.s_qty);
				$('#txtTotalMeter').val(elem.prate);
				
				$('#txtBail').val(elem.bag);
				$('#txtLS').val(elem.frate);
				$('#txtChPick').val(elem.cost);
				$('#txtChMtr').val(elem.s_rate);
				$('#txtAmount').val(elem.amount);
				$('#txtReceiveMeter').val(elem.lamount);
				
				
				$('#txtYarnBag').val(elem.bundle);
				$('#txtYarnRate').val(elem.discount);
				$('#txtYarnAmount').val(elem.s_damount);
				
				$('#bagwarp').val(elem.lrate);
				$('#bagwept').val(elem.no_of_machines);
		...
		fetchContractEdit($('#contract_no').val());
	...
	var getSaveObject = function() {
		...
		stockmain.exppercent = $.trim($('#txtIncomeTax').val());
		stockmain.expense = $.trim($('#txtIncomeTaxValue').val());
		stockmain.namount = $.trim($('#txtNetAmount').val());

		stockmain.taxpercent = $.trim($('#txtSaleTax').val());
		stockmain.tax = $.trim($('#txtSaleTaxValue').val());
		...
		if(parseInt(item_id_f)!=0){
			...
			sdstock.netamount = getNumVal($('#txtYarnAmount')) + getNumVal($('#txtNetAmount'));
			...
			sdstock.qtyf = $.trim($('#contqty').val());

			sdstock.bundle = $.trim($('#txtYarnBag').val());
			sdstock.discount = $.trim($('#txtYarnRate').val());
			sdstock.damount = $.trim($('#txtYarnAmount').val());
			sdstock.lrate = $.trim($('#bagwarp').val());
			sdstock.no_of_machines = $.trim($('#bagwept').val());
		...
		var item_id_b = getNumVal($('#hfItemId'));
		...
		var SaleTaxValue  = getNumVal($('#txtSaleTaxValue'));
		var IncomeTaxValue  = getNumVal($('#txtIncomeTaxValue'));

		amount  = parseFloat(amount)  - parseFloat(SaleTaxValue) +  parseFloat(IncomeTaxValue);
		...
		if(parseFloat(amount)!=0){
			...
			pledger.description = $('#txtPartyId').val() + ' ' + $('#remarks').val();
		...
		if(parseFloat(SaleTaxValue)!=0){
			var pledger = {};
			pledger.pledid = '';
			pledger.pid = $('#hfPartyId').val();
			pledger.description ='Sale Tax ' + $('#remarks').val();
			pledger.date = $('#vrdate').val();
			pledger.credit = parseFloat(SaleTaxValue);
			pledger.debit = 0;
			pledger.dcno = $('#txtIdHidden').val();
			pledger.invoice = $('#txtIdHidden').val();
			pledger.etype = 'frv';
			...
			pledger.description = $('#txtPartyId').val() + ' ' + $('#remarks').val();
			pledger.date = $('#vrdate').val();
			pledger.debit = parseFloat(SaleTaxValue);
			pledger.credit = 0;
			pledger.dcno = $('#txtIdHidden').val();
			pledger.invoice = $('#txtIdHidden').val();
			pledger.etype = 'frv';
		...
		if(parseFloat(IncomeTaxValue)!=0){
			var pledger = {};
			pledger.pledid = '';
			pledger.pid = $('#hfPartyId').val();
			pledger.description = 'Income Tax Deduction ' + $('#remarks').val();
			pledger.date = $('#vrdate').val();
			pledger.debit = parseFloat(IncomeTaxValue);
			pledger.credit = 0;
			pledger.dcno = $('#txtIdHidden').val();
			pledger.invoice = $('#txtIdHidden').val();
			pledger.etype = 'frv';
			pledger.pid_key = $('#incometaxid').val();
			pledger.uid = 1;
			pledger.company_id = 1;	
			pledger.isFinal = 0;
			pledger.wo = 0;
			...
			var pledger = {};
			pledger.pledid = '';
			pledger.pid = $('#incometaxid').val();
			pledger.description = $('#txtPartyId').val() + ' ' + $('#remarks').val();
			pledger.date = $('#vrdate').val();
			pledger.credit = parseFloat(IncomeTaxValue);
			pledger.debit = 0;
			pledger.dcno = $('#txtIdHidden').val();
			pledger.invoice = $('#txtIdHidden').val();
			pledger.etype = 'frv';
		...
		var amount  = getNumVal($('#txtYarnAmount'));
		if(parseFloat(amount)!=0){
			var pledger = {};
			pledger.pledid = '';
			pledger.pid = $('#wipid').val();
			pledger.description = $('#txtYarnId').val() + ' /' + $('#txtPartyId').val() + ' ' + $('#remarks').val();
			pledger.date = $('#vrdate').val();
			pledger.credit = parseFloat(amount);
			pledger.debit = 0;
			pledger.dcno = $('#txtIdHidden').val();
	...
	var clearItemData = function (){
     ...
     var CalculateUpperSum = function (){
     	...
     	var received_meter = getNumVal($('#txtTotalMeter')); 
     	...
     	var qty = parseFloat(received_meter)-parseFloat(_agrade)-parseFloat(_bgrade)-parseFloat(_cgrade);

     	
     	

     	var amount = parseFloat(qty)*parseFloat(_chmtr);
     	var IncomeTax =getNumVal($('#txtIncomeTax')); 	
     	var SaleTax =getNumVal($('#txtSaleTax')); 	
     	...
     	if(parseFloat(SaleTax)!=0 && parseFloat(amount)!=0 ){
     		SaleTaxValue= parseFloat(parseFloat(amount)*parseFloat(SaleTax)/100).toFixed(0);
     	}


     	
     	if(parseFloat(IncomeTax)!=0 && (parseFloat(amount) + parseFloat(SaleTaxValue)) !=0 ){
     		IncomeTaxValue= parseFloat((parseFloat(amount) + parseFloat(SaleTaxValue))*parseFloat(IncomeTax)/100).toFixed(0);
     	}


     	
     	NetAmount = parseFloat(parseFloat(SaleTaxValue) + parseFloat(amount) - parseFloat(IncomeTaxValue) ).toFixed(0);

     	var _conctract_qty = getNumVal($('#contqty')); 
     	
     	var con_weft_bag = getNumVal($('#txtContractWeftBag')); 
     	

     	var weft = parseFloat(parseFloat(con_weft_bag) * parseFloat(received_meter) / parseFloat(_conctract_qty));  	

     	
     	
     	var _wrapbag = getNumVal($('#txtContractWrapBag')); 
     	var wrap = parseFloat(parseFloat(_wrapbag)/parseFloat(_conctract_qty)*parseFloat(received_meter));  	

     	var totbag = parseFloat(parseFloat(wrap)+parseFloat(weft));

     	
     	
     	var _yarnrate = getNumVal($('#txtYarnRate'));
     	
     	var yarnamount = parseFloat(_yarnrate)*parseFloat(totbag); 

     	$('#txtYarnBag').val( parseFloat(totbag).toFixed(4));
     	$('#bagwarp').val(parseFloat(wrap).toFixed(4));

     	$('#bagwept').val( parseFloat(weft).toFixed(4) );



     	$('#txtYarnAmount').val(parseFloat(yarnamount).toFixed(0));
     	$('#txtIncomeTaxValue').val(IncomeTaxValue);
     	$('#txtSaleTaxValue').val(SaleTaxValue);

     	$('#txtNetAmount').val(NetAmount);
     	$('#txtAmount').val(parseFloat(amount).toFixed(0));
     	$('#txtFreshFabric').val( parseFloat(qty).toFixed(2) );
     ...
     var Print_Voucher_Account = function() {
		...
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			var url = base_url + 'index.php/reportprints/FabricReceivePrint/' + etype + '/' + vrnoa + '/' + company_id + '/' + '-1' + '/' + user + '/' + pre_bal_print+ '/' + hd + '/' + prnt + '/' + 'no' + '/' + account;
	...
	return {
		...
		bindUI : function() {
			...
			$('#ratewarp,#rateweft,#bagwarp,#bagwept,#txtIncomeTax,#txtTotalMeter,#txtAGrade,#txtBGrade,#txtCGrade,#txtFreshFabric,#txtChMtr,#txtAmount,#txtReceiveMeter,#balanceqty,#txtYarnBag,#txtYarnRate,#txtYarnAmount,#txtSaleTax').on('input', function(e) {
				e.preventDefault();
				CalculateUpperSum();
			});
			...
			$('.btnPrint').on('click', function(e) {
				...
				if ( $('.btnSave').data('printbtn')==0 ){
					...
					_height = _height - 100;
					window.open(base_url + 'application/views/reportprints/FabricReceivePrint.php', 'Fabric Receive Print', 'width='+ _width +', height='+_height);
				}
