var Advance = function() {
	...
	var getMaxId = function(etype) {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'etype' : 'advance', 'company_id':$('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(pid, name, remarks, inv, amount) {
		...
		$(row).appendTo('#advance_table');
	}

	var getCashPartyId = function() {
		var pid = "";
		$('#name_dropdown option').each(function() { if ($(this).text().trim().toLowerCase() == 'advance') pid = $(this).val();  });
		return pid;
	}
	...
	var getSaveObject = function() {
		...
		pledger.pledid = '';
		pledger.pid = _cpid;
		pledger.description = 'Advance Head';
		pledger.date = _date;
		pledger.debit = 0;
		...
		$('#advance_table').find('tbody tr').each(function() {
	...
	var search = function(from, to, etype) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$('#search_advance_table tbody tr').remove();
	...
	var populateSearchData = function(data) {
		...
		$(rows).appendTo('#search_advance_table tbody');
	}
	...
	var fetch = function(dcno, etype) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				$('#advance_table').find('tbody tr').remove();
				$('#txtNetAmount').val('');
	...
	return {
		...
		bindUI : function() {
			...
			$('#advance_table').on('click', '.btnRowRemove', function(e) {
				e.preventDefault();
			...
			$('#advance_table').on('click', '.btnRowEdit', function(e) {
				e.preventDefault();
			...
			$('#search_advance_table').on('click', '.btnRowEdit', function(e) {
				e.preventDefault();		// prevent the default behaviour of the link
				var dcno = $.trim($(this).closest('tr').find('td.dcno').text());
				fetch(dcno, 'advance');		// get the fee category detail by id
		...
		initSave : function() {
			...
			if (!error) {
				...
				var rowsCount = $('#advance_table').find('tbody tr').length;
		...
		resetVoucher : function() {
			...
			$('#txtNetAmount').val('');
			$('#search_advance_table').find('tbody tr').remove();
			$('#advance_table').find('tbody tr').remove();
			$('#voucher_type_hidden').val('new');
