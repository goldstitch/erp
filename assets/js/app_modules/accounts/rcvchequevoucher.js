 chequereceivevoucher = function() {
 	...
 	var fetch = function(dcno) {
 		$.ajax({
 			...
 			type : 'POST',
 			data : { 'dcno' : dcno, 'etype' : 'pd_receive', 'company_id': $('#cid').val() },
 			dataType : 'JSON',
	...
	var getMaxId = function() {
		...
		$.ajax({
			...
			type : 'POST',
			dataType : 'JSON',
			data : {'etype': 'pd_receive','company_id':$('#cid').val()},
			success : function(data) {
	...
	var saveLedgerPost = function ( pd_cheque ) {
		...
		party2Pledger.debit = getNumVal($('#txtAmount'))-getNumVal($('#txtTaxAmount'));
		party2Pledger.credit = 0;
		...
		if (getNumVal($('#txtTaxAmount')) != 0 ) {
			...
			party3Pledger.debit = $('#txtTaxAmount').val();
			party3Pledger.etype = 'pd_receive';
				...
				$.confirm({
					...
					buttons: {
						AutoSave: {
							...
							action: function () {
								Print_Voucher();
								chequereceivevoucher.resetVoucher();
							}},
							close: function () {
								chequereceivevoucher.resetVoucher();
							}}});
...
return {
	...
	init : function() {
		...
		// chequereceivevoucher.bindModalPartyGrid();
		chequereceivevoucher.fetchRequestedVr();
	},
...
initSaveAccount : function() {
	...
	if (!error) {
		...
		bindModalPartyGrid : function() {
			...
			chequereceivevoucher.pdTable = $('#party-lookup table').dataTable({
...
var chequereceivevoucher = new chequereceivevoucher();
chequereceivevoucher.init();