var PrepareSalary = function() {
	...
	var save = function( saveObj, dcno ) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'pledgers' :ssobjP , 'salarysheet' : ssobj, 'dcno' : dcno, 'etype' : 'wages','voucher_type_hidden':$('#voucher_type_hidden').val() },
			dataType : 'JSON',
	...
	var search = function(from, to) {
		...
		$.ajax({
			url : base_url + 'index.php/staff/getWages',
			type : 'POST',
		...
		$.each(data, function(index, elem) {
            ...
            oTable.fnAddData( [
				...
				"<span class='net_salary'>"+ calNetSalary(grossSalary, elem.penalty, elem.advance, elem.loan_deduction, elem.overtime, elem.incentive, elem.eobi, elem.insurance, elem.socialsec) +"</span>" ]*/
                    ...
                    "<span class='gsalary text-right' style='float:right;'>"+ parseFloat(get_val(elem.gross_salary) + get_val(elem.incentive) + get_val(elem.overtime) ).toFixed(0) +"</span>",
                    ...
                    "<span class='penalty hide' style='float:right;'>"+ elem.penalty +"</span>",
                    ...
                    "<span class='net_salary text-right' style='float:right;'>"+ parseFloat(elem.net_salary).toFixed(0) +"</span>"]
                    // "<span class='net_salary'>"+ calNetSalary(grossSalary, elem.penalty, elem.advance, elem.loan_deduction, elem.overtime, elem.incentive, elem.eobi, elem.insurance, elem.socialsec) +"</span>"]
			);
		});
	...
	var fetch = function(dcno) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'dcno' : dcno ,'etype':'wages','company_id':$('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var getMaxId = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'etype': 'wages','company_id':$('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
				console.log(data);
				if(data==false){
	...
	var getSaveObject = function() {
		...
		var _wages_account= $('#wagesid').val();
		var _wages_account_payable = $('#wagespayableid').val();
		...
		$('#salary_table').find('tbody tr').each(function( index, elem ) {
			...
			var _gross_salary = parseInt($(elem).find('span.gross_salary').text());
			...
			if (_loan != 0) {
				...
				pledger.dcno = _dcno;
				pledger.etype = _etype;
				pledger.pid_key = _wages_account;
			...
			// }
			_net_salary_employee += get_val(_net_salary);

			_net_salary_net += get_val(_gross_salary);
			...
			pledger = {};
			pledger.pledid = '';
			pledger.pid = _wages_account;
			pledger.description = 'Wages paid for the month of ' + _monthName;
			pledger.date = _date;
			pledger.debit = _net_salary_net;
			pledger.credit = 0;
			pledger.dcno = _dcno;
			pledger.etype = _etype;
			pledger.pid_key = _wages_account_payable;
			...
			pledger = {};
			pledger.pledid = '';
			pledger.pid = _wages_account_payable;
			pledger.description = 'Wages paid for the month of ' + _monthName;
			pledger.date = _date;
			pledger.credit = _net_salary_employee;
			pledger.debit = 0;
			pledger.dcno = _dcno;
			pledger.etype = _etype;
			pledger.pid_key = _wages_account;
...
var get_val = function(el){
	...
	var printSalarySheet = function() {
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			_height = _height - 100;
	        resetDataTable();
			window.open(base_url + 'application/views/print/wagessheet.php', 'Salarysheet', 'width='+ _width +', height='+_height);
		}
	}
    ...
    var Print_Voucher = function() {
        if ( $('.btnSave').data('printbtn')==0 ){
            ...
            var vrno = $('#txtId').val();
            var url = base_url + 'index.php/doc/pdf_salarySheet/' + vrno + '/' + 'wages' + '/' + $('#cid').val()  ;
            window.open(url);
        }
    }
