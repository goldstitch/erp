...
var saleorder = function() {
	...
	var chkexchange = function(vrnoa) {
	
		$.ajax({
			url : base_url + 'index.php/saleorder/chkexchange',
			type : 'POST',
			data : {'vrnoa':vrnoa},
			dataType : 'JSON',
			success : function(data) {
...
var Print_Voucher = function(prnt,account) {
	...
	var fetch_ = function(vrnoa) {
		
		$.ajax({

			url : base_url + 'index.php/saleorder/fetch_',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val(),'etype':'sale' ,'etype2':$('#InvType_dropdown').val()},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
			      ...
			      $('#txtNetAmount3').val(parseFloat(-data).toFixed(0));
				}
			}, error : function(xhr, status, error) {
				console.log(xhr.responseText);
			}
	...
	var populateData = function(data) {
		...
		$('#exchange').val( data[0]['exchange_id']);
	...
	var Table_Total =function(){
		...
		// if(bulkitemcheck!=='edit'){
			...
			// var exchange = 	$('#txtNetAmount3').val();
			// if(exchange =='')
			// {
			...
			// else
			// {
			// 	totIncAmount =parseFloat(totIncAmount) - parseFloat(exchange);
				
		    //     $('#txtNetAmount2').val(parseFloat(totIncAmount).toLocaleString());
			// }
			...
			$("#total_sale").val(parseFloat(totIncAmount).toFixed(0));
			$("#exchange_amount").val(parseFloat(exchange).toFixed(0));
			var sale = 	$('#total_sale').val();
            var net_amount=sale-exchange;
			$("#net_amount").val(parseFloat(net_amount).toFixed(0));
			$("#txtbalance").val(parseFloat(net_amount).toFixed(0));
			$("#txtNetAmount2").val(parseFloat(totIncAmount).toFixed(0));
	...
	return {
		...
		bindUI : function() {
			...
			$('#txtPaid').on('input', function() {
				...
				var balance =paid-net;
		    	$("#txtbalance").val(parseFloat(balance).toFixed(0));
