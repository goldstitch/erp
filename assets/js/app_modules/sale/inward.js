var partsdetail = function() {
	...
	var fetchItemStocks = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'inward'},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
					// alert('No data found.');
	...
	var fetchLfiveStocks = function(item_id) {
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'inward' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'inward','crit':crit},
			dataType : 'JSON',
			success : function(data) {
	...
	var save = function(stockmain) {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'stockmain' : stockmain.stockmain, 'stockdetail' : stockmain.stockdetail, 'vrnoa' : stockmain.vrnoa, 'ledger' : stockmain.ledger ,'voucher_type_hidden':$('#voucher_type_hidden').val(),'etype':'inward' },
			dataType : 'JSON',
			success : function(data) {
				...
				if (data.error === 'true') {
					...
					alert('Voucher hase been save successfully........');
					resetVoucher();
				}
			}, error : function(xhr, status, error) {
				console.log(xhr.responseText);
	...
	var CheckDuplicateVoucher = function(stockmain) {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'gp' : $('#txtInvNo').val(), 'vrnoa' : $('#txtVrnoaHidden').val(),'etype':'inward', 'company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
				console.log(data);
				if (data === false) {
	...
	var Print_Voucher = function(hd) {
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			// var url = base_url + 'index.php/doc/Print_Voucher/' + etype + '/' + vrnoa + '/' + company_id + '/' + '-1' + '/' + user + '/' + pre_bal_print+ '/' + hd + '/' + 'lg' + '/' + '1' ;
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val(),'etype':'inward'},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchOgp = function(vrnoa) {

		$.ajax({

			url : base_url + 'index.php/inward/fetchOgp',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#unit_dropdown').val(), 'company_id2': $('#cid').val(),'etype':'outward'},
			dataType : 'JSON',
			success : function(data) {
				...
				$('#txtOgp').val(vrnoa);
				if (data === 'false') {
					alert('No pending data found against this OGP#.');
				} else {
					
					if(data[0]['duplicate_vrnoa']=='00'){
						populateDataOgp(data);
					}else{
						alert('Duplicate Ogp found at: ' +data[0]['duplicate_vrnoa'] );
						return false;
					}
				}
	...
	fetchPurchaseOrder = function(vrnoa) {

		$.ajax({

			url : base_url + 'index.php/purchaseorder/fetchRemaining',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val(),'etype':$('#orderType_dropdown').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data) {
		...
		$('#unit_dropdown').select2('val',data[0]['officer_id']);
		$('#txtOgp').val(data[0]['order_no']);
		...
		$.each(data, function(index, elem) {
			appendToTable('', elem.item_name, elem.item_id, Math.abs(elem.s_qty), elem.rate, elem.amount,Math.abs(elem.weight), elem.type,elem.dozen,elem.bag,elem.job_id,elem.item_name_article,elem.s_qtyf,elem.bundle,elem.frate);
	...
	var populateDataPO = function(data) {
		...
		$.each(data, function(index, elem) {
			var dozen=0
			if(elem.uom=='dozen'){
				dozen=parseFloat(parseFloat(elem.st_qty)/12).toFixed(2) ;
			}
			appendToTable('', elem.item_name, elem.item_id, elem.st_qty, elem.rate,elem.amount ,elem.st_weight, elem.type,dozen,0,elem.item_id_cus,elem.item_name_article,elem.s_qty,elem.st_qty1,elem.st_qty);
	...
	var populateDataOgp = function(data) {
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'inward'},
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id, qty, rate, amount, weight, tbl,dzn_qty,bag,item_id_article,item_desc_article,orderqty,received,balqty) {
		...
		"<td class='orderqty numeric text-right' data-title='orderqty' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTOrderQty text-right' disabled=''  value='"+ orderqty +"'></td>" +
		"<td class='received numeric text-right' data-title='received' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTRecQty text-right' disabled=''  value='"+ received +"'></td>" +
		"<td class='balqty numeric text-right' data-title='balqty' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTBalQty text-right' disabled=''  value='"+ balqty +"'></td>" +
		...
		"<td class='rate hidden numeric text-right' data-title='rate' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTRate text-right'  value='"+ rate +"'></td>" +
	...
	var calculateNewValues = function ()
	{
		...
		$('.txtTQty,.txtTRate,.txtTWeight,.txtTFedRate').on('input', function ()
		{
		...
		$('.txtTQty,.txtTRate,.txtTDznQty,.txtTWeight,.txtTotalBag,.txtTOrderQty,.txtTRecQty,.txtTBalQty').on('focus', function (e)
		{
			e.preventDefault();
			$(this).select();
		});
	...
	var appendToTable_2nd = function(srno, item_desc, item_id, qty, rate, amount, weight, typee,stqty,stweight) {
      ...
      "<td class='weight numeric' data-title='Weigh' > "+ weight +"</td>" +
					 ...
					 var getSaveObject = function() {
					 	...
					 	stockmain.officer_id = $('#unit_dropdown').val();
					 	stockmain.order_vrno = $('#txtOgp').val();
					 	...
					 	$('#purchase_table').find('tbody tr').each(function( index, elem ) {
							...
							sdstock.job_id = $.trim($(this).closest('tr').find('td.item_desc_article').data('item_id'));
					 		
					 		
					 		sdstock.dozen = $.trim($(elem).find('input.txtTDznQty').val());
					 		...
					 		sdstock.bag = $.trim($(elem).find('input.txtTBag').val());


					 		sdstock.qtyf = $.trim($(elem).find('input.txtTOrderQty').val());
					 		sdstock.bundle = $.trim($(elem).find('input.txtTRecQty').val());
					 		sdstock.frate = $.trim($(elem).find('input.txtTBalQty').val());
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'inward','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
...
var resetFields = function() {
	...
	return {
		...
		bindUI : function() {
			...
			$('#txtOgp').on('keypress', function(e) {
				...
				if (e.keyCode === 13) {
					...
					if ($(this).val() != '') {
						...
						e.preventDefault();
						var _unit = $.trim($('#unit_dropdown').val());
						if ( !_unit ) {
							$('#unit_dropdown').addClass('inputerror');
							alert('Please select unit first and try again.....');
						}else{
							fetchOgp($(this).val());	
						}
			...
			$('#txtPo').on('input', function(e) {
					
				if ($(this).val() != '') {

					e.preventDefault();
					var _odertype = $.trim($('#orderType_dropdown').val());
					if ( !_odertype ) 
					{
						$('#orderType_dropdown').addClass('inputerror');
						alert('Please select order type first and try again.....');
					}
					else
					{
						fetchPurchaseOrder($(this).val());
					}	
				}
			});
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					var received = $('#txtRecQty').val();
					var balqty = $('#txtBalQty').val();

					

					appendToTable('', item_desc, item_id, qty, rate, amount, weight,"add",dozen,bag,item_id_article,item_desc_article,orderqty,received,balqty);
			...
			$('#purchase_table').on('click', '.btnRowEdit', function(e) {
				...
				var orderqty = $.trim($(this).closest('tr').find('input.txtTOrderQty').val());
				var received = $.trim($(this).closest('tr').find('input.txtTRecQty').val());
				var balqty = $.trim($(this).closest('tr').find('input.txtTBalQty').val());
				...
				$('#txtRecQty').val(received);
				$('#txtBalQty').val(balqty);
