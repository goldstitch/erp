...
var fetchAccount = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'active' : 1,'typee':'partsdetail'},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
					alert('No data found');
...
var getSaveObjectItem = function() {
	...
	var save = function(saleorder) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'ordermain' : saleorder.ordermain, 'orderdetail' : saleorder.orderdetail, 'vrnoa' : saleorder.vrnoa, 'ledger' : saleorder.ledger ,'voucher_type_hidden':$('#voucher_type_hidden').val(),'etype':'order_parts' },
			dataType : 'JSON',
			success : function(data) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val(),'etype':'order_parts'},
			dataType : 'JSON',
			success : function(data) {
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'order_parts'},
			dataType : 'JSON',
			success : function(data) {
	...
	var Validate_Order = function(ord) {
		$.ajax({
			url : base_url + 'index.php/saleorder/Validate_Order',
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'sale_order','order_no':ord,'status':'running'},
			dataType : 'JSON',
			success : function(data) {
				 var ord =data;
				if (isNaN(ord) || ord==false ){
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'order_parts','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchThroughPO = function(po) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/saleorder/fetchPartsOrder',
			type : 'POST',
	...
	var populatePOData = function(data) {
		...
		if(data['less']!=='false'){
			$.each(data['less'], function(index, elem) {
			appendToTable('', elem.item_name, elem.item_id, elem.qty, elem.rate, elem.amount, elem.weight, 'less');
			});
		}
		
		if(data['parts']!=='false'){
			$.each(data['parts'], function(index, elem) {
				appendToTable('', elem.item_name, elem.item_id, elem.qty, elem.rate, elem.amount, elem.weight, 'parts');
			});
		}
		if(data['spare']!=='false'){
			$.each(data['spare'], function(index, elem) {
				appendToTable('', elem.item_name, elem.item_id, elem.qty, elem.rate, elem.amount, elem.weight, 'spare parts');
			});
		}
		Remove_Less_Items();
	}
		...
		bindUI : function() {
			...
			$('.modal-lookup .populateOrder').on('click', function(){
				...
				if(order_id!=0){
					...
					var ord = Validate_Order(order_id);
				}
			});
			...
			$('#txtOrderNo').on('keypress', function(e) {
				...
				if (e.keyCode === 13) {
					if ($(this).val() != '') {
						e.preventDefault();
						var ord = Validate_Order($(this).val());
