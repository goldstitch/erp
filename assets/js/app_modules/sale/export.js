var saleorder = function() {
    ...
    var fetchTypeParty = function(type) {
        $.ajax({
            ...
            url : base_url + 'index.php/export/fetchTypeParty',
            type : 'POST',
    ...
    var save = function(saleorder) {
        ...
        $.ajax({
            url : base_url + 'index.php/export/save',
            type : 'POST',
            data : { 'ordermain' : saleorder.ordermain, 'orderdetail' : saleorder.orderdetail, 'vrnoa' : saleorder.vrnoa, 'ledger' : saleorder.ledger ,'voucher_type_hidden':$('#voucher_type_hidden').val() ,'etype':'export'},
            dataType : 'JSON',
            success : function(data) {
    ...
    var fetchReports = function (from, to,companyid,etype,uid) {
        ...
        $.ajax({
            ...
            data: { 'from' : from, 'to' : to, 'company_id':companyid, 'etype':'export', 'uid':uid},
            type: 'POST',
            dataType: 'JSON',
            beforeSend: function () {
    ...
    var Print_Voucher = function(prnt,account) {
        if ( $('.btnSave').data('printbtn')==0 ){
            ...
            var wrate=0;
            if(prnt=='comercial'){
                var url = base_url + 'index.php/doc/Print_Order_Voucher/sale/' + $('#txtSaleInv').val() + '/' + company_id + '/' + '-1' + '/' + user + '/' + pre_bal_print+ '/' + hd + '/' + prnt + '/' + 'no' + '/' + account;
            }else{
                var url = base_url + 'index.php/doc/Print_Export_Voucher/' + etype + '/' + vrnoa + '/' + company_id + '/' + '-1' + '/' + user + '/' + pre_bal_print+ '/' + hd + '/' + prnt + '/' + 'no' + '/' + account;    
            }
    ...
    var fetch = function(vrnoa) {
        ...
        $.ajax({
            ...
            url : base_url + 'index.php/export/fetch',
            type : 'POST',
            data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val(),'etype':'export'},
            dataType : 'JSON',
    ...
    var fetch_vrno = function(vrno) {
        ...
        $.ajax({
            ...
            url : base_url + 'index.php/export/fetch_vrno',
            type : 'POST',
    ...
    var last_5_srate = function(party_id,item_id) {
        ...
        $.ajax({
            ...
            url : base_url + 'index.php/export/last_5_srate',
            type : 'POST',
    ...
    var populateData = function(data) {
        ...
        $('#InvType_dropdown').val(data['main'][0]['etype2']);
        // $('#InvType_dropdown').disabled = true;
        $("#InvType_dropdown").attr("disabled", true);
        $('#txtVrno').val(data['main'][0]['vrno']);
        $('#txtVrnoHidden').val(data['main'][0]['vrno']);
        $('#txtVrnoaHidden').val(data['main'][0]['vrnoa']);
        $('#txtVrnoa').val(data['main'][0]['vrnoa']);
        $('#current_date').val(data['main'][0]['vrdate'].substring(0,10));
        $('#party_dropdown').select2('val', data['main'][0]['party_id']);

        $('#txtInvNo').val(data['main'][0]['bilty_no']);
        $('#due_date').val(data['main'][0]['bilty_date'].substring(0,10));
        $('#receivers_list').val(data['main'][0]['received_by']);
        $('#transporter_dropdown').select2('val', data['main'][0]['transporter_id']);
        $('#txtRemarks').val(data['main'][0]['remarks']);
        $('#txtNetAmount').val(data['main'][0]['namount']);
        $('#txtOrderNo').val(data['main'][0]['ordno']);

        $('#txtDiscount').val(data['main'][0]['discp']);
        $('#txtExpense').val(data['main'][0]['exppercent']);
        $('#txtExpAmount').val(data['main'][0]['expense']);
        $('#txtTax').val(data['main'][0]['taxpercent']);
        $('#txtTaxAmount').val(data['main'][0]['tax']);
        $('#txtDiscAmount').val(data['main'][0]['discount']);
        $('#user_dropdown').val(data['main'][0]['uid']);
        // $('#txtPaid').val(data[0]['paid']);

        $('#dept_dropdown').select2('val', data['main'][0]['godown_id']);
        $('#voucher_type_hidden').val('edit');
        $('#currency_dropdown').select2('val',data['main'][0]['currencey_id']);
        $('#txtExchangeRate').val(data['main'][0]['lprate_m']);

        $('#txtContainer').val(data['main'][0]['container_no']);
        $('#txtGrossWeight').val(data['main'][0]['gross_weight']);
        $('#txtNetWeight').val(data['main'][0]['net_weight']);
        $('#txtLcNo').val(data['main'][0]['lc_no']);
        $('#lc_date').val(data['main'][0]['lc_date'].substring(0,10));
        $('#txtSealNo').val(data['main'][0]['seal_no']);
        $('#txteformNo').val(data['main'][0]['eform_no']);
        $('#edate').val(data['main'][0]['edate'].substring(0,10));

        $('#txtShipmentFrom').val(data['main'][0]['shippment_from']);
        $('#txtShipmentTo').val(data['main'][0]['shippment_to']);
        // $('#txtCPONo').val(data[0]['cpono']);
        // $('#txtTaxStatus').val(data[0]['tax_status']);
        $('#txtDeliveryTerm').val(data['main'][0]['delivery_term']);
        $('#txtPaymentTerm').val(data['main'][0]['payment_term']);
        $('#txtExportRegistrationNo').val(data['main'][0]['export_register_no']);
        $('#txtExportMadeIn').val(data['main'][0]['made_in']);
        $('#txtPortofDischarge').val(data['main'][0]['port_of_discharge']);
        $('#txtConsigneetotheorderof').val(data['main'][0]['cosignee_the_order_of']);
        $('#txtSaleInv').val(data['main'][0]['sale_invoice_no']);

        $('#hfConsigneeCompanyName').val(data['main'][0]['party_name']);
        $('#hfConsigneeCompanyAddress').val(data['main'][0]['party_address']);
        $('#hfConsigneeCompanyPhone').val(data['main'][0]['party_phone']);

        $('#txtVessel').val(data['main'][0]['vessel']);
        $('#txtVoyage').val(data['main'][0]['voyage']);
        $('#txtBlNo').val(data['main'][0]['bl_no']);
        $('#txtRemarksPacking').val(data['main'][0]['remarkspacking']);
        $('#txtRemarksBl').val(data['main'][0]['remarksbl']);
        $('#txtBlQty').val(data['main'][0]['blqty']);
        $('#txtCif').val(data['main'][0]['cif']);
        $('#txtHsCode').val(data['main'][0]['hscode']);
        $('#txtNotifyParty').val(data['main'][0]['notify_party']);

        

        if(data['main'][0]['bl_date'] != null) {
            $('#txtBlDate').val(data['main'][0]['bl_date'].substring(0, 10));
        }


        $.each(data.main, function(index, elem) {
            appendToTable('', elem.item_name, elem.item_id, Math.abs(elem.qty), elem.rate, elem.amount, Math.abs(elem.weight), elem.type,elem.dozen,elem.item_id_cus,elem.artcile_no_cus,elem.item_desc_cus,elem.frate,elem.artcile_no,elem.gstrate,elem.gstamount,elem.netamount_d,elem.ctn_qty,  elem.ctnmarking, elem.origin_criterion, elem.noofctn, elem.color, elem.size);
        });
    ...
    var getMaxVrno = function() {
        ...
        $.ajax({
            ...
            url : base_url + 'index.php/export/getMaxVrno',
            type : 'POST',
            data : {'company_id': $('#cid').val(),'etype':'export'},
            dataType : 'JSON',
            success : function(data) {
    ...
    var getMaxVrnoa = function() {
        ...
        $.ajax({
            ...
            url : base_url + 'index.php/export/getMaxVrnoa',
            type : 'POST',
            data : {'company_id': $('#cid').val() ,'etype':'export' },
            dataType : 'JSON',
            success : function(data) {
    ...
    var validateSingleProductAddpacking = function() {
        ...
        var errorFlag = false;
        var itemEl = $('#item_dropdown_packing');
        var ctnEl = $('#txtTotalCtns');
        var dznEl = $('#txtTotalDozens');
        ...
        if ( !ctnEl.val() ) {
            ctnEl.addClass('inputerror');
            errorFlag = true;
        }
        if ( !dznEl.val() ) {
            dznEl.addClass('inputerror');
            errorFlag = true;
        }
    ...
    var appendToTable = function(srno, item_desc, item_id, qty, rate, amount, weight, tbl,dzn_qty,item_id_cus,article_no_cus,item_desc_cus,frate,article_no,gstrate,gstamount,namount,ctn, ctnmarking, origionCriterion, noOfCartoon, color, size) {
        ...
        var row = 	"<tr>" +
            ...
            "<td class='ctnmarking numeric text-right' data-title='PO' > <input class='form-control input-sm ' type='text' id='txtCtnMarking' style='width:50px;' value='" + ctnmarking + "'> </td>" +
            ...
            "<td class='origincriterion numeric text-right' data-title='Origin criterion' > <input class='form-control input-sm ' type='text' id='txtQriginCriterion' value='" + origionCriterion + "'> </td>" +
            "<td class='noofcarton numeric text-right hide' data-title='No of carton' > <input class='form-control input-sm num' type='text' id='txtNoOfCartoon' value='" + noOfCartoon + "'> </td>" +
            "<td class='color numeric text-right' data-title='Color' > <input class='form-control input-sm ' type='text' id='txtColor' value='" + color + "'> </td>" +
            "<td class='size numeric text-right' data-title='Size' > <input class='form-control input-sm ' type='text' id='txtSize' value='" + size + "'> </td>" +
            "</tr>";
    ...
    var appendToTablePacking = function(srno, ctnmarking,item_desc, item_id, color, size, noofctn, totalctns, totaldoznes, dpr) {

        var srno = $('#packinglist_table tbody tr').length + 1;
 
        var row =   "<tr>" +
            "<td class='srno numeric' data-title='Sr#' > "+ srno +"</td>" +
            "<td class='ctnmarking' data-title='ctnmarking'>  "+ ctnmarking +"</td>" +
            "<td class='item_desc' data-title='Description' data-item_id='"+ item_id +"'> "+ item_desc +"</td>" +
            "<td class='color' data-title='Color'>  "+ color +"</td>" +
            "<td class='size ' data-title='Size'>  "+ size +"</td>" +
            "<td class='noofctn numeric text-right' data-title='NoofCtn'>  "+ noofctn +"</td>" +
            "<td class='totalctns numeric text-right' data-title='TotalCarton' > "+ totalctns +"</td>" +
            "<td class='totaldoznes numeric text-right' data-title='Total Doznes'> "+ totaldoznes +"</td>" +
            "<td class='dpr numeric text-right' data-title='DPR'> "+ dpr +"</td>" +
        ...
        $(row).appendTo('#packinglist_table');
    ...
    var getSaveObject = function() {
        ...
        ordermain.godown_id = $('#dept_dropdown').val();
        ...
        ordermain.made_in = $('#txtExportMadeIn').val();
        ordermain.port_of_discharge = $('#txtPortofDischarge').val();
        ordermain.cosignee_the_order_of = $('#txtConsigneetotheorderof').val();
        ordermain.sale_invoice_no = $('#txtSaleInv').val();
        ordermain.vessel = $('#txtVessel').val();
        ordermain.voyage = $('#txtVoyage').val();
        ordermain.bl_no = $('#txtBlNo').val();
        ordermain.bl_date = $('#txtBlDate').val();
        ordermain.remarkspacking = $('#txtRemarksPacking').val();
        ordermain.remarksbl = $('#txtRemarksBl').val();
        ordermain.hscode = $('#txtHsCode').val();
        ordermain.blqty = $('#txtBlQty').val();
        ordermain.cif = $('#txtCif').val();
        ordermain.notify_party = $('#txtNotifyParty').val();
        ...
        $('#purchase_table').find('tbody tr').each(function( index, elem ) {
            ...
            sd.type="add";
            sd.ctnmarking = $.trim($(elem).find('td.ctnmarking input').val());
            sd.origin_criterion = $.trim($(elem).find('td.origincriterion input').val());
            sd.noofctn = $.trim($(elem).find('td.noofcarton input').val());
            sd.color = $.trim($(elem).find('td.color input').val());
            sd.size = $.trim($(elem).find('td.size input').val());
            orderdetail.push(sd);
        });
        $('#packinglist_table').find('tbody tr').each(function( index, elem ) {
            var sd = {};
            sd.oid = '';
            sd.item_id_packing = $.trim($(elem).find('td.item_desc').data('item_id'));
            sd.ctnmarking = $.trim($(this).closest('tr').find('td.ctnmarking').text());
            sd.colors = $.trim($(this).closest('tr').find('td.color').text());
            sd.size = $.trim($(this).closest('tr').find('td.size').text());
            sd.noofctn = $.trim($(this).closest('tr').find('td.noofctn').text());
            sd.totalctns = $.trim($(this).closest('tr').find('td.totalctns').text());
            sd.totaldoznes = $.trim($(this).closest('tr').find('td.totaldoznes').text());
            sd.dpr = $.trim($(this).closest('tr').find('td.dpr').text());
            orderdetail.push(sd);
        });
    ...
    var fetchLfiveStocks = function(item_id) {
        $.ajax({
            url : base_url + 'index.php/export/fetchLfiveStocks',
            type : 'POST',
    ...
    var fetchLfiveRates = function(item_id) {
        ...
        $.ajax({
            url : base_url + 'index.php/export/fetchLfiveRates',
            type : 'POST',
    ...
    var deleteVoucher = function(vrnoa) {
        ...
        $.ajax({
            url : base_url + 'index.php/export/delete',
            type : 'POST',
            data : { 'vrnoa' : vrnoa , 'etype':'export','company_id':$('#cid').val() },
            dataType : 'JSON',
            success : function(data) {
    ...
    var calculateLowerTotalPacking = function(dozen) {


        var _dozen = getNumText($('.txtTotalPDozen'));
        
        var tempDozen = parseFloat(_dozen) + parseFloat(dozen);
        $('.txtTotalPDozen').text(tempDozen);
    ...
    var fetchThroughPO = function(po) {
        ...
        $.ajax({
            ...
            data : { 'vrnoa' : po , 'company_id': $('#cid').val(),'etype':'sale','etype2':'export'},
            dataType : 'JSON',
            success : function(data) {
                resetFields();
                $('#txtSaleInv').val(po);
    ...
    var populatePOData = function(data) {
        ...
        $('#e_date').val(data[0]['edate'].substring(0,10));
    ...
    var resetFields = function() {
        ...
        $('#txtBlQty').val('');
        $('#txtRemarksPacking').val('');
        $('#txtConsigneetotheorderof').val('');
    ...
    return {
        ...
        bindUI : function() {
            ...
            $('.btnPrintBL').on('click',  function(e) {
                e.preventDefault();
                Print_Voucher('BL');
                //window.open(base_url + 'application/views/reportprints/vouchers_reports_bl.php', "BL Report", 'width=970, height=auto,scrollbars=1');
            });

            $('.btnPrintCoo').on('click',  function(e) {
                e.preventDefault();
                window.open(base_url + 'application/views/reportprints/vouchers_reports_coo.php', "Certificate of Origin Report", 'width=970, height=auto,scrollbars=1');
            });

            $('.btnPrintComercial').on('click',  function(e) {
                e.preventDefault();
                Print_Voucher('comercial');
                //window.open(base_url + 'application/views/reportprints/vouchers_reports_comercial.php', "Commercial Report", 'width=970, height=auto,scrollbars=1');
            });

            $('.btnprintpacking').on('click', function(e) {
                e.preventDefault();
                Print_Voucher('Packing');
                //window.open(base_url + 'application/views/reportprints/vouchers_reports_packing.php', "Packing Report", 'width=970, height=auto,scrollbars=1');
            });

            $('.btnPrintGsp').on('click', function(e) {
                e.preventDefault();
               window.open(base_url + 'application/views/reportprints/vouchers_reports_gsp.php', "GSP Report", 'width=970, height=auto,scrollbars=1');
            });
            ...
            $('#txtSaleInv').on('keypress', function(e) {
            ...
            $('#txtDPR,#txtTotalDozens,#txtTotalCtns').on('input', function() {
                
                var _ctn = getNumVal($('#txtTotalCtns'));
                var _dozen=getNumVal($('#txtTotalDozens'));

                if (_ctn != '0' && _dozen !='0') {
                    var _tempAmnt = parseFloat(_dozen) / parseFloat(_ctn);
                
                    $('#txtDPR').val(parseFloat(_tempAmnt).toFixed(2));
                }
            ...
            $('#btnAddPackingList').on('click', function(e) {
                e.preventDefault();

                var error = validateSingleProductAddpacking();
                if (!error) {

                    var item_desc = $('#item_dropdown_packing').find('option:selected').text();
                    var item_id = $('#item_dropdown_packing').val();
                    var ctnMarking = $('#txtCtnMarking').val();
                    var color = $('#txtColour').val();
                    var size = $('#txtSize').val();
                    var noofctn = $('#txtNoOfCtn').val();
                    var totalctns = $('#txtTotalCtns').val();
                    var totaldoznes = $('#txtTotalDozens').val();
                    var dpr = $('#txtDPR').val();

                    // reset the values of the annoying fields
                    $('#item_dropdown_packing').select2('val', '');
                    ...
                    appendToTablePacking('', po,item_desc, item_id, color, size, noofctn, totalctns, totaldoznes, dpr);
                    calculateLowerTotalPacking(totaldoznes);
                    $('#stqty_lbl').text('Item');
                    $('#item_dropdown').focus();
                } else {
                    alert('Correct the errors!');
            ...
            $('#packinglist_table').on('click', '.btnRowRemove', function(e) {
                e.preventDefault();

                var dozen = $.trim($(this).closest('tr').find('td.totaldoznes').text());
                
                calculateLowerTotalPacking("-"+dozen);
                $(this).closest('tr').remove();
            });
            $('#packinglist_table').on('click', '.btnRowEdit', function(e) {
                e.preventDefault();

                // getting values of the cruel row
                var item_id = $.trim($(this).closest('tr').find('td.item_desc').data('item_id'));
                var ctnMarking = $.trim($(this).closest('tr').find('td.ctnmarking').text());
                var color = $.trim($(this).closest('tr').find('td.color').text());
                var size = $.trim($(this).closest('tr').find('td.size').text());
                var noofctn = $.trim($(this).closest('tr').find('td.noofctn').text());
                var totalctns = $.trim($(this).closest('tr').find('td.totalctns').text());
                var totaldoznes = $.trim($(this).closest('tr').find('td.totaldoznes').text());
                var dpr = $.trim($(this).closest('tr').find('td.dpr').text());
                

                $('#item_dropdown_packing').select2('val', item_id);
                $('#txtCtnMarking').val(ctnMarking);
                $('#txtColour').val(color);
                $('#txtSize').val(size);
                $('#txtNoOfCtn').val(noofctn);
                $('#txtTotalCtns').val(totalctns);
                $('#txtTotalDozens').val(totaldoznes);
                $('#txtDPR').val(dpr);
