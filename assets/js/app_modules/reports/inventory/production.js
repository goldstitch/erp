 Purchase = function() {
    ...
    var fetchVouchers = function (from, to, what, type,etype,field,crit,orderBy,groupBy,name) {
        ...
        $.ajax({
                url: base_url + "index.php/report/fetchPurchaseReportData_production",//fetchSaleProductionData
                ...
                success: function (result) {
                    ...
                    if (result.length !== 0 || result.length !== '') {
                        ...
                        if (type == "detailed" || type=='summary') {
                            ...
                            var salary_net=0;
                            var salary_diff=0;
                            var salary_diff_net=0;
                            var sal_desc='';
                            var date_diff=0;
                            var pre_date='';


                            // date_diff= (general.datediff(from,to,'days'))+1;
                            ...
                            // date_diff= (date_diff==0)?1:date_diff;
                                ...
                                $.each(result, function (index, elem) {
                                    ...
                                    obj.NAME = (elem.staff_name) ? elem.staff_name : "-";
                                    ...
                                    obj.NETAMOUNT = (elem.lamount) ? elem.lamount : "-";
                                    ...
                                    if (what=='voucher'){
                                        ...
                                        prevVoucherMatch=elem.workorder;
                                    }else if(what=='agreement'){
                                        prevVoucherMatch=elem.agreement;
                                    }else if(what=='type'){
                                        prevVoucherMatch=elem.type;
                                    }else if(what=='person'){
                                        prevVoucherMatch=elem.staff_name;
                                    }else if(what=='employee'){
                                        prevVoucherMatch=elem.staff_name;
                                    }else if(what=='shift'){
                                        prevVoucherMatch=elem.shift_name;
                                    }else if(what=='article'){
                                        prevVoucherMatch=elem.article;
                                    }else if(what=='phase'){
                                    ...
                                    if (prevVoucher != prevVoucherMatch) {
                                        if (index !== 0) {
                                            ...
                                            if(what=='employee'){
                                                ...
                                                salary_gross = (salary_gross!=0 ) ? (salary_gross/30)*date_diff : 0;
                                                salary_net += parseFloat(salary_gross);

                                                salary_diff=parseFloat(parseFloat(salary_gross) - parseFloat(totalSum)).toFixed(2);
                                                salary_diff_net += parseFloat(salary_diff);
                                                sal_desc= 'Salary: ' +parseInt(salary_gross) + ',   Diff:' + parseInt(salary_diff) + ', ';
                                                date_diff=0;
                                            } 
                                            var source   = $("#voucher-sum-template").html();
                                            var template = Handlebars.compile(source);
                                            var html = template({'TOTAL_SALARY':sal_desc ,VOUCHER_SUM : totalSum.toFixed(2), VOUCHER_QTY_SUM : Math.abs(totalQty).toFixed(2), VOUCHER_WEIGHT_SUM : Math.abs(totalWeight).toFixed(2),'TOTAL_HEAD':'TOTAL' });
                                    ...
                                    salary_gross = (elem.staff_salary) ? elem.staff_salary : 0;
                                    
                                    if(elem.vrdate!=pre_date){
                                        date_diff +=1;
                                        pre_date= elem.vrdate;
                                    }

                                    

                                    grandTotal += parseFloat(elem.lamount);
                                    grandQty += parseInt(elem.qty);
                                    grandWeight += parseFloat(elem.weight);

                                    totalSum+= parseFloat(elem.lamount);
                                    ...
                                    if (index === (result.length -1)) {
                                            ...
                                            if(what=='employee'){
                                                ...
                                                var sal_desc_net= 'Total Salary: ' +parseInt(salary_net) + ',   Diff:' + parseInt(salary_diff_net) + ', ';
                                            } 
                                        ...
                                        var source   = $("#voucher-sum-template").html();
                                        var template = Handlebars.compile(source);
                                        var html = template({'TOTAL_SALARY':sal_desc_net ,VOUCHER_SUM : grandTotal.toFixed(2), VOUCHER_QTY_SUM : Math.abs(grandQty).toFixed(2), VOUCHER_WEIGHT_SUM : Math.abs(grandWeight).toFixed(2),'TOTAL_HEAD':'GRAND TOTAL' });
    ...
    var resetd = function () {
     ...
     var getcrit = function (etype){
        ...
        var txtphase=$('#drpphaseID').select2("val");
        // End Items
        // Account
        ...
        // var stafftype=$('#drpStaffType').select2("val");
        ...
        var crit ='';
            ...
            if (staid!=''){
                crit +='AND stockdetail.emp_id in (' + staid +') ';
            }
            if (stafftype!=''){
               
                crit +="AND staff.type in ('" + stafftype +"') ";
            }
            if (itemid!='') {
            ...
            if (txtphase!='') {
                crit +='AND stockdetail.phase_id in (' + txtphase +') ';
            }
            if (txtUom!='') {
                // alert('"'+txtUom+'"'); 
            ...
            // }
            //End Account
            var d_type = ($('#Radio3').is(':checked') ? 'produce' : 'consume');

            crit += "AND stockmain.stid <>0 AND stockdetail.type ='" + d_type + "' ";
            // alert(crit);
        return crit;
        ...
        bindUI : function() {
            ...
            $('#btnSearch').on('click', function(e) {
                ...
                if (what === 'voucher') {
                    ...
                    field =   'stockmain.received_by';
                    orderBy = 'stockmain.received_by';
                    groupBy = 'stockmain.received_by';
                    name    = 'stockmain.received_by';
                }else if (what === 'godown') {
                    field =   'dept.name';
                    orderBy = 'dept.name';
                    groupBy = 'dept.name';
                    ...
                    field =   'stockdetail.order';
                    orderBy = 'stockdetail.order';
                    groupBy = 'stockdetail.order';
                    name    = 'staff.name';
                }else if (what === 'employee') {
                    field =   'staff.name';
                    orderBy = 'staff.name';
                    ...
                    field =   'item.artcile_no';
                    orderBy = 'item.artcile_no';
                    groupBy = 'item.artcile_no';
                    name    = 'item.artcile_no';
                }else if (what === 'phase') {
                    field =   'subphase.name';
                    orderBy = 'subphase.name';
                    groupBy = 'subphase.name';
                    name    = 'subphase.name';
                }
