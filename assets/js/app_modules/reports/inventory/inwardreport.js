 Purchase = function() {
    ...
    var fetchVouchers = function (from, to, what, type,etype,field,crit,orderBy,groupBy,name) {
        ...
        $('#saleRows').empty();
            ...
            $.ajax({
                url: base_url + "index.php/report/fetchPurchaseReportData_inward",//fetchSaleProductionData
                    ...
                    if (result.length !== 0 || result.length !== '') {
                        ...
                        if (type !== "summary") {
                            ...
                            $.each(result, function (index, elem) {
                                    ...
                                    obj.NAME = (elem.party_name) ? elem.party_name : "-";
                                    ...
                                    if(elem.etype=='consumption'){
                                        obj.HREFF = base_url +'index.php/consumption?vrnoa=' + elem.vrnoa;
                                    }else if(elem.etype=='inward'){
                                        obj.HREFF = base_url +'index.php/inward?vrnoa=' + elem.vrnoa;
                                    }else if(elem.etype=='outward'){
                                        obj.HREFF = base_url +'index.php/inward/outward?vrnoa=' + elem.vrnoa;
                                    ...
                                    if (what=='voucher'){
                                        ...
                                        prevVoucherMatch=elem.barcode;
                                    }else if(what=='article'){
                                        prevVoucherMatch=elem.artcile_no;
                                    }
...
var fetchchartOrders = function (from, to, what, type,etype,field,crit,orderBy,groupBy,name) {
    ...
    var donut_data=[];
        ...
        if (accid!=''){
            ...
            //End Account
            var d_type = ($('#Radio3').is(':checked') ? 'all' : 'other');
            if(d_type=='all'){
                crit += "AND stockmain.stid <>0 ";
            }else{
                d_type = ($('#Radio1').is(':checked') ? 'returnable' : 'non returnable');
                crit += "AND stockmain.stid <>0 AND stockmain.etype2 ='" + d_type + "' ";
            }
 ...
 $('#btnSearch').on('click', function(e) {
    ...
    var to = $('#to_date').val();
    var what = getCurrentView();
                var type = ($('#Radio1').is(':checked') ? 'returnable' : 'nonreturnable');     // if true means detailed view if false sumamry view
                // alert(type;)
                var etype=($('#etype').val().trim()).toLowerCase();
                etype=etype.replace(' ','');
                // alert(etype);
