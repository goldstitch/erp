Purchase = function() {
    ...
    var fetchVouchers = function (from, to, what, type,etype,crit) {
        ...
        $.ajax({
            url: base_url + "index.php/yarnissue/fetchYarnReportData",
            data: { 'from' : from, 'to' : to, 'what' : what, 'type' : type, 'company_id':$('#cid').val(),'etype':etype,'crit':crit},
            type: 'POST',
            dataType: 'JSON',
            beforeSend: function () {
                    ...
                    if (type == "detailed") {
                        ...
                        $("#datatable_example_wrapper").fadeIn();
                            ...
                            $.each(result, function (index, elem) {
                                ...
                                obj.QTY = (elem.bag) ? Math.abs(elem.bag) : "-";
                                obj.WEIGHT = (elem.qty) ? Math.abs(elem.qty) : "-";
                                ...
                                if (prevVoucher != elem.voucher) {
                                    ...
                                    var template = Handlebars.compile(source);
                                    var html = template({VRNOA1 : elem.voucher });
                                ...
                                grandQty += parseInt(elem.bag);
                                grandWeight += parseFloat(elem.qty);

                                totalSum+= parseFloat(elem.netamount);
                                totalQty += parseInt(elem.bag);
                                totalWeight += parseFloat(elem.qty);
...
$('#btnSearch').on('click', function(e) {
    ...
    fetchVouchers(from, to, what, type,etype,crit);    
