...
var validateSaveItem = function() {
	...
	var save = function(purchase) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'voucher_type_hidden':$('#voucher_type_hidden').val(),  'stockmain' : purchase.stockmain, 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa, 'ledger' : JSON.stringify(purchase.ledger),'etype':'vb'  },
			dataType : 'JSON',
			success : function(data) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val() ,'etype':'vb'},
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			appendToTable('', elem.item_name, elem.item_id, elem.qty, elem.rate, elem.amount, elem.weight,elem.gstp,elem.gst,elem.fedp,elem.fed,elem.dozen,elem.uom,elem.inventory_id,elem.dwork_orderno,elem.parchi,elem.type,elem.label);
	...
	var populateDataIgp = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			appendToTable('', elem.item_name, elem.item_id, elem.s_qty, elem.cost_price,amount , elem.weight,0,0,0,0,elem.dozen,elem.uom,elem.inventory_id,elem.workorder);
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			type : 'POST',
			data : {'company_id': $('#cid').val(),'etype':'vb'},
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id, qty, rate, amount, weight,gstp,gst,fedp,fed,dozen,uom,inventory_id,workorderno,vr,etype,vr_date) {
		...
		"<td class='fedp numeric text-right hide' data-title='Fed%' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTFedRate text-right'  value='"+ fedp +"'></td>" +
		
		"<td class='fed numeric text-right hide' data-title='fed'> "+ fed +"</td>" +
		...
		"<td class='vr  text-left' data-title='Vr#' > "+ vr +"</td>" +
		"<td class='etype  text-left' data-title='Type' > "+ etype +"</td>" +
		"<td class='vr_date  text-left' data-title='Date' > "+ vr_date +"</td>" +
	...
	var calculateNewValues = function ()
	{
		...
		$('.txtTQty,.txtTRate,.txtWeight,.txtTFedRate,.txtTGstRate').on('input', function ()
		{
			...
			$(this).closest('tr').find('td.fed').text(fedamount);
	...
	var getSaveObject = function() {
		...
		$('#purchase_table').find('tbody tr').each(function( index, elem ) {
			...
			sd.type = $.trim($(elem).find('td.etype').text());
			sd.parchi = $.trim($(elem).find('td.vr').text());
			sd.label = $.trim($(elem).find('td.vr_date').text());
			...
			if(parseFloat(sd.amount) - parseFloat(sd.gst) !=0){
				var pledger = {};
				pledger.pledid = '';
				...
				pledger.debit = parseFloat(sd.amount) - parseFloat(sd.gst);
				pledger.credit = 0;
			...
			if( parseFloat(sd.gst) !=0){
				...
				pledger.date = $('#current_date').val();
				pledger.debit =  parseFloat(sd.gst);
				pledger.credit = 0;
				pledger.dcno = $('#txtVrnoaHidden').val();
				pledger.invoice = $('#txtInvNo').val();
				pledger.etype = 'vb';
		...
		var tax_amount =getNumVal($('#txtTaxAmount'));


		if( parseFloat(getNumVal($('#txtNetAmount'))) + parseFloat(tax_amount) !=0){
			var pledger = {};
			pledger.pledid = '';
			...
			pledger.date = $('#current_date').val();
			pledger.debit = 0;
			pledger.credit = parseFloat(parseFloat(getNumVal($('#txtNetAmount'))) + parseFloat(tax_amount)).toFixed(0);
			pledger.dcno = $('#txtVrnoaHidden').val();
		...
		if ($('#txtTaxAmount').val() != 0 ) {
			...
			var pledger = {};
			pledger.etype = 'vb';
			pledger.description = $('#txtPartyId').val() +', Inv'+ $('#txtInvNo').val();
			// pledger.description = 'Purchase Head';
			pledger.dcno = $('#txtVrnoaHidden').val();
			pledger.invoice = $('#txtVrnoaHidden').val();
			...
			var pledger = {};
			pledger.etype = 'vb';
			pledger.description = 'Income Tax Deduction. ' + $('#txtRemarks').val() +', Inv'+ $('#txtInvNo').val();
			// pledger.description = 'Purchase Head';
			pledger.dcno = $('#txtVrnoaHidden').val();
			pledger.invoice = $('#txtVrnoaHidden').val();
			pledger.pid = $('#hfPartyId').val();
	...
	var fetchItemStocks = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'vb'},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
					// alert('No data found.');
	...
	var fetchLfiveStocks = function(item_id) {
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'vb' ,'vrdate':$('#current_date').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': "'vb','yarnPurchase','fabricPurchase'",'crit':crit},
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTableLfiveRates = function(vrnoa,vrdate,qty,lprate) {
		...
		"<td class='lprate text-right' data-title='Description' data-lprate='"+ lprate +"'> "+ parseFloat(lprate).toFixed(2) +"</td>" +
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'vb','company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var calculateUpperSum = function() {
		...
		var _gst = 0;//getNumVal($('#txtgst'));
		var _fed = 0;//getNumVal($('#txtfed'));




		var fedp = getNumVal($('#txtfedp'));
		var gstp = getNumVal($('#txtgstp'));

		
		var amount= parseFloat(_qty) * parseFloat(_prate) ;
		

		if(amount!==0 && fedp!==0){
			_fed= parseFloat(parseFloat(amount)*parseFloat(fedp) /100 ).toFixed(0) ;
		}



		if(amount!==0 && gstp!==0){
			_gst= parseFloat(parseFloat(amount)*parseFloat(gstp)/100).toFixed(0);
		}
	...
	var fetchThroughIgp = function(po) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : po , 'company_id': $('#cid').val(),'etype':'inward','etype2':'pur_order'},
			dataType : 'JSON',
			success : function(data) {
	...
	var populatePOData = function(data) {
		...
		$('#user_dropdown').val(data[0]['uid']);
		$.each(data, function(index, elem) {
			appendToTable('', elem.item_name, elem.item_id, elem.qty, elem.rate, elem.amount, elem.weight,0,0,0,0,elem.workorder,elem.vrnoa,elem.etype,elem.vrdate);
	...
	var getAllBills = function() {
		...
		$('#datatable_example tr').each(function(index, elem) {
			...
			var item_id = $(elem).find('td.item').data('item_id');

			var uom = $(elem).find('td.item').data('uom');
			var inventory_id = $(elem).find('td.item').data('inventory_id');


			var item_des = $(elem).find('td.item').text();
			var wo = $(elem).find('td.vrnoa').data('wo');
			var etype = $(elem).find('td.etype').text();
			var vrdate = $(elem).find('td.vrdate').text();

			var qty = $(elem).find('td.qty').text();
			var weight = $(elem).find('td.weight').text();
			var rate = $(elem).find('td.rate').text();
			var amount = $(elem).find('td.amount').text();



			
			var vrnoa = $(elem).find('td.vrnoa').data('vrnoa');
			...
			if (chk=='on'){
				...
				appendToTable('',item_des, item_id, qty, rate, amount, weight,0,0,0,0,0,uom,inventory_id,wo,vrnoa,etype,vrdate);
	...
	var AddAllPayments = function() {
		...
		$('#datatable_example tbody tr').each(function(index, elem) {
			...
			var chk = $(elem).find('td input[type="checkbox"]:checked').val();
			totalAmount = parseFloat($(elem).find('td.amount').text().trim());
			vrnoa = parseFloat($(elem).find('td.vrnoa').text().trim());
	...
	return {
		...
		bindUI : function() {
			...
			// $('#txtExpiryDate').on('keyup', function(e) {
			// 	e.preventDefault();
			// 	$('#txtExpiryDate').removeClass('inputerror');
			// 	var v = this.value;
			// 	if (v.match(/^\d{2}$/) !== null) {
			// 		this.value = v + '/';
			...
			// 		if(retVal===false){
			// 			$('#txtExpiryDate').addClass('inputerror');
...
$('#txtgstp,#txtfedp').on('input', function() {
...
$('#txtgst,#txtfed').on('blur',function(e){
	e.preventDefault();
	calculateUpperSum();
});
...
$('#txtLevel3').on('change', function() {
	...
	if (level3 !== "" && level3 !== null) {
		...
		fetchBillData : function ( from, to, reportType, pid ) {
			...
			$.ajax({
				url : base_url + 'index.php/report/fetchVendorBillData',
				method : 'POST',
				dataType : 'JSON',
				data : { from : '2000-01-01', to : $('#current_date').val(), reportType : '', party_id : getNumVal($('#hfPartyId')), company_id : $('#cid').val() ,'crit':'' },
				beforeSend : function () { },
				success : function ( result ) {
					...
					if (result.length !== 0) {
						...
						$(result).each(function(index, elem){
							...
							if(parseFloat(elem.balance)!=0){
								var source   = $( '#voucher-bill-item-template' ).html();
								var template = Handlebars.compile(source);
