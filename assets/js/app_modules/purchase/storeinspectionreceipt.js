var Purchase = function() {
	...
	var save = function(sir) {

		$.ajax({
			url : base_url + 'index.php/storeinspectionreceipt/save',
			type : 'POST',
			data : { 'stockmain' : sir.stockmain, 'stockdetail' : sir.stockdetail, 'vrnoa' : sir.vrnoa },
			dataType : 'JSON',
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/storeinspectionreceipt/fetch',
			type : 'POST',
			data : { 'vrnoa' : vrnoa },
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			appendToTable('1', elem.item_name, elem.item_id, elem.dept_name, elem.godown_id, elem.uom, elem.qty, elem.qtyf, elem.item_rate, elem.item_amount);
			calculateVoucherNetAmount(elem.qty, elem.item_amount);		
		});
	}
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/storeinspectionreceipt/getMaxVrno',
			type : 'POST',
			dataType : 'JSON',
			success : function(data) {
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/storeinspectionreceipt/getMaxVrnoa',
			type : 'POST',
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id, dept, dept_id, qtyRec, qtyRej, qtyApp, prate, amount) {

		var row = 	"<tr>" +
						"<td class='srno'> "+ srno +"</td>" +
				 		"<td class='item' data-item_id='"+ item_id +"'> "+ item_desc +"</td>" +
				 		"<td class='dept' data-dept_id='"+ dept_id +"'> "+ dept +"</td>" +
				 		"<td class='qtyRec'> "+ qtyRec +"</td>" +
				 		"<td class='qtyRej'> "+ qtyRej +"</td>" +
				 		"<td class='qtyApp'> "+ qtyApp +"</td>" +
	...
	var getSaveObject = function() {
		...
		$('#purchase_table').find('tbody tr').each(function( index, elem ) {
			...
			sd.uom = $.trim($(elem).find('td.qtyRec').text());
			sd.qtyf = $.trim($(elem).find('td.qtyRej').text());
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/storeinspectionreceipt/delete',
			type : 'POST',
			data : { 'vrnoa' : vrnoa },
			dataType : 'JSON',
			success : function(data) {
	...
	////////////////////////////////////////////////////////////////
	var calculateVoucherNetAmount = function(qtyApp, amount) {

		var _qty = ($('#txtGQty').val() == "") ? 0 : $('#txtGQty').val();
		var _amnt = ($('#txtGAmnt').val() == "") ? 0 : $('#txtGAmnt').val();

		var tempQty = parseFloat(_qty) + parseFloat(qtyApp);
		$('#txtGQty').val(tempQty);
	...
	var fetchThroughIGP = function(IGPNo) {
	...
	var populateDataIGP = function(data) {
		$.each(data, function(index, elem) {
			var net = parseFloat(elem.cost_price) * parseFloat(elem.qty);
			appendToTable('1', elem.item_name, elem.item_id, elem.dept_name, elem.godown_id, elem.qty, '-', elem.qty, elem.cost_price, net);
			calculateVoucherNetAmount(elem.qty, net);		
		});
	}
		...
		bindUI : function() {
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					var dept_id = $('#dept_dropdown').val();
					var qtyRec = $('#txtQtyRec').val();
					var qtyRej = $('#txtQtyRej').val();
					var qtyApp = $('#txtQtyApp').val();
					...
					appendToTable('1', item_desc, item_id, dept, dept_id, qtyRec, qtyRej, qtyApp, prate, amount);
					calculateVoucherNetAmount(qtyApp, amount);
				} else {
			...
			$('#purchase_table').on('click', '.btnRowRemove', function(e) {
				e.preventDefault();
				var qtyApp = $.trim($(this).closest('tr').find('td.qtyApp').text());
			...
			$('#purchase_table').on('click', '.btnRowEdit', function(e) {
				...
				var qtyRec = $.trim($(this).closest('tr').find('td.qtyRec').text());
				var qtyRej = $.trim($(this).closest('tr').find('td.qtyRej').text());
				var qtyApp = $.trim($(this).closest('tr').find('td.qtyApp').text());
				...
				$('#txtQtyRec').val(qtyRec);
				$('#txtQtyRej').val(qtyRej);
				$('#txtQtyApp').val(qtyApp);
				$('#txtPrate').val(prate);
			...
			$('#txtIGPNo').on('keypress', function(e) {
				if (e.keyCode === 13) {
					var IGPNo = $('#txtIGPNo').val();
					if (IGPNo !== '') {
						fetchThroughIGP(IGPNo);
					}
				}
			});
