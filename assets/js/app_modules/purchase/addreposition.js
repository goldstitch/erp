...
var getSaveObjectItem = function() {
	...
	var save = function(purchaseorder) {
		...
		$.ajax({
			url : base_url + 'index.php/reposition/save',
			type : 'POST',
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/reposition/fetch',
			type : 'POST',
	...
	var fetchItemStocks = function(item_id) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/reposition/fetchItemStocks',
			type : 'POST',
	...
	var fetchLfiveStocks = function(item_id) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/reposition/fetchLfiveStocks',
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/reposition/fetchLfiveRates',
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			appendToTable('', elem.item_name, elem.item_id, elem.qty, elem.weight, elem.rate, elem.lvendor,elem.lstock);
			calculateLowerTotal(elem.qty, elem.weight);
		});
	}
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/reposition/getMaxVrno',
			type : 'POST',
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/reposition/getMaxVrnoa',
			type : 'POST',
			data : {'company_id': $('#cid').val()},
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id, qty ,weight, rate, vendor, stock) {
		...
		var row = 	"<tr>" +
						"<td class='srno numeric' data-title='Sr#' > "+ srno +"</td>" +
				 		...
				 		"<td class='weight numeric' data-title='weight'>  "+ weight +"</td>" +
					 	"<td class='rate numeric' data-title='Rate'> "+ rate +"</td>" +
					 	"<td class='vendor numeric' data-title='vendor' > "+ vendor +"</td>" +
	...
	var getSaveObject = function() {
		...
		$('#purchase_table').find('tbody tr').each(function( index, elem ) {
			...
			sd.lvendor = $.trim($(elem).find('td.vendor').text());
		...
		bindUI : function() {
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					appendToTable('', item_desc, item_id, qty,weight, rate, vendor, stock);
