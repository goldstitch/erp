var FabricPurchase = function() {
	...
	var save = function(sale) {
		...
		$.ajax({
			url : base_url + 'index.php/fabricPurchase/save',
			type : 'POST',
	...
	var Print_Voucher = function(hd,account) {
		...
		if ( $('.btnSave').data('printbtn')==0 ){
			...
			// alert(account);
			// var hd = $('#hd').val();
			var pre_bal_print = 0;//($(settings.switchPreBal).bootstrapSwitch('state') === true) ? '0' : '1';
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			url : base_url + 'index.php/fabricPurchase/fetch',
			type : 'POST',
	...
	var populateDataIgp = function(data) {
		...
		$('#user_dropdown').val(data[0]['uid']);
		$.each(data, function(index, elem) {
			appendToTable('', elem.item_name, elem.item_id,elem.uom,elem.s_qty,0,0, elem.s_qty,0, elem.cost_price,parseFloat(parseFloat(elem.cost_price)*parseFloat(elem.s_qty)).toFixed(2) , elem.weight,0,0,0,0,elem.workorder);
			calculateLowerTotal(elem.s_qty, parseFloat(parseFloat(elem.cost_price)*parseFloat(elem.s_qty)).toFixed(2), elem.weight);
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			...
			appendToTable('', elem.item_name, elem.item_id,elem.uom,elem.pick,elem.reed,elem.width,elem.qty,elem.count, elem.rate, elem.amount,elem.weight,elem.dwork_orderno);
	...
	var getMaxVrno = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/fabricPurchase/getMaxVrno',
			type : 'POST',
	...
	var getMaxVrnoa = function() {
		...
		$.ajax({
			...
			url : base_url + 'index.php/fabricPurchase/getMaxVrnoa',
			type : 'POST',
			data : {'company_id': $('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var appendToTable = function(srno, item_desc, item_id,uom,pick,reed,width,qty,count, rate, amount, weight,workorderno) {
		...
		var row = 	"<tr>" +
						"<td class='srno numeric text-right' data-title='Sr#' > "+ srno +"</td>" +
				 		...
				 		"<td class='count numeric text-right' data-title='Count'>  "+ count +"</td>" +
					 	"<td class='pick text-right' data-title='Pick'>  "+ pick +"</td>" +
					 	"<td class='reed text-right' data-title='Reed'>  "+ reed +"</td>" +
					 	"<td class='width text-right' data-title='Width'>  "+ width +"</td>" +
...
var getSaveObjectItem = function() {
	...
	var getSaveObject = function() {
		...
		stockmain.etype 		= 'fabricPurchase';
		...
		$('#sale_table').find('tbody tr').each(function( index, elem ) {
			...
			sd.pick     = $.trim($(elem).find('td.pick').text());
			sd.reed	    = ($.trim($(elem).find('td.reed').text()));
			sd.width	    = ($.trim($(elem).find('td.width').text()));
		...
		pledger.etype = 'fabricPurchase';
		pledger.pid_key = $('#fabricpurchaseid').val();
		pledger.uid = $('#uid').val();
		...
		var pledger = {};
		pledger.pledid = '';
		pledger.pid = $('#fabricpurchaseid').val();
		...
		if ($('#txtFreight').val() != 0 ) {
			...
			var pledger = {};
			pledger.etype = 'fabricPurchase';
			pledger.description = $('#party_dropdown option:selected').text() + '. Freight Charges.  ' + $('#txtRemarks').val();
			// pledger.description = 'Purchase Head';
			pledger.dcno = $('#txtVrnoaHidden').val();
			pledger.invoice = $('#txtVrnoaHidden').val();
	...
	var deleteVoucher = function(vrnoa) {
		...
		$.ajax({
			url : base_url + 'index.php/fabricPurchase/delete',
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'etype':'fabricPurchase','company_id':$('#cid').val()},//, 'company_id':$('#cid').val() },
			dataType : 'JSON',
			success : function(data) {
	...
	var calculateLowerTotal = function(qty, amount, weight) {
		...
		var tempQty = parseFloat(parseFloat(_qty) + parseFloat(qty)).toFixed(2);
		...
		var net = parseFloat(tempAmnt) - parseFloat(_disc) + parseFloat(_taxamount) + parseFloat(_expense) - parseFloat(_frieght) ;
	...
	var fetchThroughIgp = function(po) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : po , 'company_id': $('#cid').val(),'etype':'inward','etype2':'fabricPurchaseContract'},
			dataType : 'JSON',
			success : function(data) {
	...
	var fetchItemStocks = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'fabricPurchase'},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
					// alert('No data found.');
	...
	var fetchLfiveRates = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': "'purchase','yarnPurchase','fabricPurchase'"},
			dataType : 'JSON',
			success : function(data) {
	...
	return {
		...
		bindUI : function() {
			...
			$('#party_dropdown,#broker,#itemid_dropdown,#item_dropdown,#txtColor,#txtWono,#category_dropdown,#subcategory_dropdown,#brand_dropdown,#txtLevel3').select2();
   ...
   //  			$('#ItemAddModel').modal('show');
			...
			$('#btnAdd').on('click', function(e) {
				...
				if (!error) {
					...
					//var colors 		= $('#txtColor').find('option:selected').text();
					//var colorid  	= $('#txtColor').val();
					var qty    		= $('#txtQty').val();
					//var quality    	= $('#txtQlty').val();
					//var brand  		= $('#txtBrand').val();
					var count  		= $('#txtBolt').val();
					var pick  		= $('#txtQtyGross').val();
					var reed  		= $('#txtLess').val();
					var width  		= $('#txtShortage').val();
					var uom     	=  $('#txtUom').val();
					...
					appendToTable('', item_desc, item_id,uom,pick,reed,width,qty,count, rate, amount, weight,workorderno);
			...
			$('#sale_table').on('click', '.btnRowEdit', function(e) {
				...
				var pick 	= $.trim($(this).closest('tr').find('td.pick').text());
				var reed 	= $.trim($(this).closest('tr').find('td.reed').text());
				var width 	= $.trim($(this).closest('tr').find('td.width').text());
				var workorderno 	= $.trim($(this).closest('tr').find('td.workorderno').text());


				$('#itemid_dropdown').select2('val', item_id);
				$('#item_dropdown').select2('val', item_id);
				//$('#txtColor').select2('val', color);
				...
				$('#txtUom').val(uom);
				$('#txtLess').val(reed);
				$('#txtShortage').val(width);
				$('#txtQtyGross').val(pick);
				$('#txtQty').val(qty);
				//$('#txtBrand').val(brand);
				$('#txtBolt').val(count);
				$('#txtPRate').val(rate);
		...
		initSaveItem : function() {
			...
			if (!error) {
				...
				bindModalPartyGrid : function() {
				            ...
				            fabric.pdTable = $('#party-lookup table').dataTable({
...
bindModalItemGrid : function() {
				            ...
				            fabric.pdTable = $('#item-lookup table').dataTable({
				                // "sDom": "<'row-fluid table_top_bar'<'span12'>'<'to_hide_phone'>'f'<'>r>t<'row-fluid'>",
				                "sDom": "<'row-fluid table_top_bar'<'span12'<'to_hide_phone' f>>>t<'row-fluid control-group full top' <'span4 to_hide_tablet'l><'span8 pagination'p>>",
				                "aaSorting": [[0, "asc"]],
				                "bPaginate": true,
