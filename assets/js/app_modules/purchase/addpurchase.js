...
var validateSaveItem = function() {
	...
	var save = function(purchase) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'voucher_type_hidden':$('#voucher_type_hidden').val(),  'stockmain' : purchase.stockmain, 'stockdetail' : purchase.stockdetail, 'vrnoa' : purchase.vrnoa, 'ledger' : JSON.stringify(purchase.ledger),'etype':'purchase'  },
			dataType : 'JSON',
			success : function(data) {
	...
	var fetch = function(vrnoa) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'vrnoa' : vrnoa , 'company_id': $('#cid').val() ,'etype':'purchase'},
			dataType : 'JSON',
			success : function(data) {
	...
	var populateData = function(data) {
		...
		$.each(data, function(index, elem) {
			appendToTable('', elem.item_name, elem.item_id, elem.qty, elem.rate, elem.amount, elem.weight,elem.gstp,elem.gst,elem.fedp,elem.fed,elem.dozen,elem.uom,elem.inventory_id,elem.dwork_orderno);
	...
	var appendToTable = function(srno, item_desc, item_id, qty, rate, amount, weight,gstp,gst,fedp,fed,dozen,uom,inventory_id,workorderno) {
		...
		"<td class='dzn_qty numeric text-right' data-title='dzn_qty' style='text-align: right; max-width:60px;'> <input type='text' class='form-control num txtTDznQty text-right' disabled='' value='"+ dozen +"'></td>" +
		...
		"<td class=''><a href='' class='btn btn-primary btnRowEdit'><span class='fa fa-edit'></span></a> <a href='' class='btn btn-primary btnRowRemove'><span class='fa fa-trash-o'></span></a> </td>" +
		"</tr>";
	...
	var getSaveObject = function() {
		...
		$('#purchase_table').find('tbody tr').each(function( index, elem ) {
			...
			if(parseFloat(sd.amount) - parseFloat(sd.gst) - parseFloat(sd.fed) !=0){
				var pledger = {};
				pledger.pledid = '';
				...
				pledger.debit = parseFloat(parseFloat(sd.amount) - parseFloat(sd.gst) - parseFloat(sd.fed) ).toFixed(2);
				pledger.credit = 0;
		...
		if( parseFloat(getNumVal($('#txtNetAmount'))) !=0){
			var pledger = {};
			pledger.pledid = '';
		...
		console.log(getNumText($('.txtTotalGst')));
		...
		if ( getNumText($('.txtTotalGst')) != 0 ) {
			...
			pledger.date = $('#current_date').val();
			pledger.debit = getNumText($('.txtTotalGst'));
			pledger.credit = 0;
			pledger.isFinal = 0;
			pledger.uid = $('#uid').val();
			pledger.company_id = $('#cid').val();	
		...
		if ( getNumText($('.txtTotalFed')) != 0 ) {
			...
			pledger.date = $('#current_date').val();
			pledger.debit = getNumText($('.txtTotalFed'));
			pledger.credit = 0;
			pledger.isFinal = 0;
			pledger.uid = $('#uid').val();
			pledger.company_id = $('#cid').val();	
	...
	var fetchItemStocks = function(item_id) {
		...
		$.ajax({
			...
			type : 'POST',
			data : { 'item_id' : item_id , 'company_id': $('#cid').val(),'etype': 'purchase'},
			dataType : 'JSON',
			success : function(data) {
				if (data === 'false') {
					// alert('No data found.');
