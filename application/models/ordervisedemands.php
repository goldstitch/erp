...
class Ordervisedemands extends CI_Model {
    ...
    public function fetch($vrnoa, $etype, $company_id) {
        $result = $this->db->query("SELECT d.qty,d.weight,d.amount,d.rate,subp.id as subpahase_id ,subp.name as subphase_name,m.finishedItem_id,d.rate2,d.etype AS detype,d.calculationmethod,d.uom, m.party_id_co,m.currency_id, d.received_by AS 'received',d.workdetail,g.name AS dept_name,m.vrno,m.uid,m.paid, m.vrnoa,m.vrdate,m.taxpercent,m.exppercent,m.tax,m.discp,m.discount, m.party_id, m.bilty_no AS 'inv_no', m.bilty_date AS 'due_date', m.received_by, m.transporter_id, m.remarks, ROUND(m.namount, 2) namount, m.order_vrno AS 'order_no', ROUND(m.freight, 2) freight, ROUND(m.discp, 2) discp, ROUND(m.discount, 2) discount, ROUND(m.expense, 2) expense, m.vehicle_id AS 'amnt_paid', m.officer_id, ROUND(m.ddays) AS 'due_days', d.item_id, d.godown_id, ROUND(d.qty, 2) AS 's_qty', ROUND(d.qtyf, 2) AS s_qtyf, ROUND(d.rate, 2) AS 's_rate', ROUND(d.amount, 2) AS 's_amount', ROUND(d.damount, 2) AS 's_damount', ROUND(d.discount, 2) AS 's_discount', ROUND(d.netamount, 2) AS 's_net', i.item_des AS 'item_name', d.weight FROM itemmain AS m INNER JOIN itemdetail AS d ON m.stid = d.stid LEFT JOIN item AS i ON i.item_id = d.item_id LEFT JOIN department AS g ON g.did = d.godown_id LEFT JOIN subphase AS subp ON subp.id = d.subphase_id WHERE m.vrnoa = $vrnoa AND m.company_id = $company_id AND m.etype = '" . $etype . "'");
        if ($result->num_rows() > 0) {
            return $result->result_array();
        } else {
            return false;
