...
class FabricPurchase extends CI_Controller {
...
unauth_secure();
CheckVoucherRights('fabricpurchase');
$data['modules'] 	= array('purchase/addFabricPurchase');
...
$this->load->view('purchase/addFabricPurchase',$data);
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->orders->getMaxVrno('fabricPurchase',$company_id) +1;
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->orders->getMaxVrnoa('fabricPurchase',$company_id) +1;
$this->output->set_content_type('application/json')->set_output(json_encode($result));
}
}
...
$igpStatus = $this->orders->igpStatus($vrnoa,$stockmain['order_vrno'],'fabricPurchase',$stockmain['company_id'],$voucher_type_hidden);
if($igpStatus == false) {
if ($voucher_type_hidden=='new'){
$vrnoa = $this->orders->getMaxVrnoa('fabricPurchase',$stockmain['company_id']) +1;
}
$result = $this->ledgers->save($ledger,$vrnoa,'fabricPurchase',$voucher_type_hidden);
$result = $this->orders->save($stockmain,$stockdetail,$vrnoa,'fabricPurchase');
$response = array();
if ( $result === false ) {
$response['error'] = 'true';
}else {
...
$vrnoa = $_POST['vrnoa'];
$company_id =1;
$result = $this->orders->fetch($vrnoa,'fabricPurchase',$company_id);
$response = "";
if ( $result === false ) {
$response = 'false';
}else {
