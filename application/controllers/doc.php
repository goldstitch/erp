...
class Doc extends CI_Controller {
    ...
    public function Account_Voucher($etype, $vrnoa, $company_id, $email_pdf = - 1, $user, $hd = 1) {
        ...
        if ($etype == 'purchase') {
            ...
            $data['title'] = 'Arhat Voucher';
        } elseif ($etype == 'discountpaid') {
            $data['title'] = 'Discount Paid Voucher';
        } elseif ($etype == 'tr_produce') {
            $data['title'] = 'Vendor Stock Transfer';
        } elseif ($etype == 'rfv') {
            $data['title'] = 'Receive From Vendor';
        } elseif ($etype == 'itv') {
            $data['title'] = 'Issue To Vendor';
        } else {
            $data['title'] = $etype;
        }
        $data['etype'] = $etype;
        ...
        $data['vrdetail'] = '';
        $data['vrdetail'] = $this->ledgers->fetchAccountVoucher($vrnoa, $etype, $company_id);
        $this->wkhtmltopdf->addPage($this->load->view('reportprints/AccountVoucherPrint', $data, true));
        $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait', 'footer-spacing' => '3.0', 'margin-left' => '5', 'margin-right' => '5', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        if ($email_pdf == 1) {
            $party_email = $data['vrdetail'][0]['party_email'];
            $party_name = $data['vrdetail'][0]['party_name'];
            $vrdate = $data['vrdetail'][0]['vrdate'];
            $vrnoa = $data['vrdetail'][0]['vrnoa'];
            $body_messegage = "Dear! " . $party_name . " find your " . $data['title'] . " #" . $vrnoa . " Dated: " . date('d-M-y', strtotime($vrdate)) . " in attachemt.. Thanks! ";
            if ($party_email != "") {
                $save_path = ($_SERVER['DOCUMENT_ROOT'] . '/bi/assets/documents/Voucher.pdf');
                $this->wkhtmltopdf->saveAs($save_path);
                sleep(5);
                $this->send_mail($save_path, $data['title'], $body_messegage, $party_email);
            } else {
                echo "Sorry! No Email Found Against this Party";
            }
        } else {
    ...
    public function pdf_doublecheque($etype, $vrnoa, $company_id) {
        ...
        $this->wkhtmltopdf->setOptions(array('orientation' => 'landscape', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        $this->wkhtmltopdf->send();
    }
    public function send_mail($filePath, $subject, $message) {
        $configs = array('protocol' => 'smtp', 'smtp_host' => 'server1.alnaharsolutions.co.uk', 'smtp_user' => 'info@afaqtraders.com', 'smtp_pass' => "info@afaqtraders", 'smtp_port' => '465');
...
");
    ...
    public function Account_Flow($dt1, $dt2, $party_id, $company_id, $email_pdf = - 1, $user_print) {
        ...
        $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        if ($email_pdf == 1) {
    ...
    public function pdf_TrialBalance6($dt1, $dt2, $company_id, $email_pdf = - 1, $user_print, $l1, $l2, $l3, $with_zero) {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['pledger'] = $this->accounts->fetchTrialBalanceData6($dt1, $dt2, $company_id, $l1, $l2, $l3, $with_zero);
        ...
        $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait', 'margin-left' => '5', 'margin-right' => '5', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        if ($email_pdf == 1) {
            $save_path = ($_SERVER['DOCUMENT_ROOT'] . '/AfaqTraders/application/assets/documents/' . microtime() . 'Voucher.pdf');
            $this->wkhtmltopdf->saveAs($save_path);
            sleep(5);
    ...
    public function pdf_TrialBalance($dt1, $dt2, $company_id, $email_pdf = - 1, $user_print, $l1, $l2, $l3, $with_zero) {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['pledger'] = $this->accounts->fetchTrialBalanceData6($dt1, $dt2, $company_id, $l1, $l2, $l3, $with_zero);
        $data['title'] = 'Trial Balance';
    ...
    public function Item_Ledger_Pdf($dt1, $dt2, $item_id, $company_id, $email_pdf = - 1, $user_print, $pid) {
        ...
        $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9', 'margin-left' => '5', 'margin-right' => '5'));
        if ($email_pdf == 1) {
            $save_path = ($_SERVER['DOCUMENT_ROOT'] . '/AfaqTraders/application/assets/documents/' . microtime() . 'Voucher.pdf');
            $this->wkhtmltopdf->saveAs($save_path);
            sleep(5);
    ...
    public function Print_Order_Voucher($etype, $vrnoa, $company_id, $email_pdf = - 1, $user, $pre_bal_print, $hd = 1, $prnt = 'lg', $wrate = '', $account) {
        ...
        if ($etype == 'pur_order') {
            ...
            $data['title'] = 'General Contract';
        } elseif ($etype == 'consumption') {
            $data['title'] = 'Consumption Voucher';
        } elseif ($etype == 'yarnissue') {
        ...
        if ($prnt == 'comercial') {
            ...
            $this->wkhtmltopdf->setOptions(array('orientation' => 'Portrait', 'page-size' => 'Letter', 'footer-spacing' => '5.0', 'header-spacing' => '5.0', 'footer-html' => base_url() . 'assets/temppdffooter/' . $footerFileName, 'header-html' => base_url() . 'assets/temppdffooter/' . $headerFileName, 'margin-left' => '3', 'margin-right' => '3'));
        } else if ($etype == 'glovescontract') {
            $this->wkhtmltopdf->setOptions(array('orientation' => 'landscape', 'margin-left' => '5', 'margin-right' => '5', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        } else {
            $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        }
        if ($prnt == 'comercial') {
    ...
    public function Item_Required_Material_Print($etype, $vrnoa, $company_id, $email_pdf = - 1, $user, $pre_bal_print, $hd = 1, $prnt = 'lg', $wrate = '') {
        ...
        $data['vrdetailLabour'] = $this->orderitemmaterials->fetchLabour($vrnoa, $etype, $company_id, 'labour');
        $data['vrdetailEbelishment'] = $this->orderitemmaterials->fetchLabour($vrnoa, $etype, $company_id, 'embelishment');
        $data['vrdetailFabrication'] = $this->orderitemmaterials->fetchLabour($vrnoa, $etype, $company_id, 'fabrication');
        $data['VoucherSummary'] = $this->orderitemmaterials->fetchVoucherSummary($vrnoa, $etype, $company_id);
        $data['ArticleSummary'] = $this->orderitemmaterials->fetchArticleSummary($vrnoa, $etype, $company_id);
    ...
    public function PrintFabricDemand($etype, $vrnoa, $company_id, $email_pdf = - 1, $user, $pre_bal_print, $hd = 1, $item_id = '0') {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['vrdetail'] = $this->orderitemmaterials->fetchFabricDemand($vrnoa, $etype, $company_id, $item_id);
        $data['YarnSummary'] = $this->orderitemmaterials->fetchOrder($data['vrdetail'][0]['worder'], $etype, $company_id, ' AND i.catid in (10) ');
        $data['FabricYarnSummary'] = $this->orderitemmaterials->fetchFabricYarnSummary($vrnoa, $etype, $company_id);
        if ($data['YarnSummary'] == false) {
            $data['YarnSummary'] = [];
        }
        $data['title'] = 'Fabric Demand';
        $data['pre_bal_print'] = $pre_bal_print;
        $data['user'] = $user;
        $data['hd'] = $hd;
        if ($hd == 1) {
        ...
        $this->wkhtmltopdf->addPage($this->load->view('reportprints/PrintFabricDemand', $data, true));
        $this->wkhtmltopdf->setOptions(array('orientation' => 'landscape', 'footer-spacing' => '3.0', 'margin-left' => '5', 'margin-right' => '5', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        if ($email_pdf == 1) {
            $save_path = ($_SERVER['DOCUMENT_ROOT'] . '/AfaqTraders/application/assets/documents/' . microtime() . 'Voucher.pdf');
            $this->wkhtmltopdf->saveAs($save_path);
            sleep(5);
    ...
    public function Print_Voucher($etype, $vrnoa, $company_id, $email_pdf = - 1, $user, $pre_bal_print, $hd = 1, $prnt = 'lg', $wrate = '1', $account) {
        ...
        if ($etype == 'purchase') {
        ...
        } elseif ($etype == 'Sample_Material_Required') {
            $data['title'] = 'Sample_Material_Required';
        } elseif ($etype == 'itv') {
            $data['title'] = 'Issue To Vendor';
        } elseif ($etype == 'tr_produce') {
            $data['title'] = 'Vendor Stock Transfer';
            ...
            $data['title'] = 'Settelment Vendor';
        } elseif ($etype == 'rfv') {
            $data['title'] = 'Receive From Vendor';
        } elseif ($etype == 'rejection') {
            $data['title'] = 'Rejection Vendor';
        } elseif ($etype == 'inward') {
            $data['title'] = 'Inward Gate Pass';
        } elseif ($etype == 'outward') {
            $data['title'] = 'Outward Gate Pass';
        }elseif ($etype == 'issue') {
            $data['title'] = 'Production Issuance Voucher';
        }elseif ($etype == 'receive') {
            $data['title'] = 'Production Receive Voucher';
        }elseif ($etype == 'sample_issue') {
            $data['title'] = 'Sample Issuance Voucher';
        }elseif ($etype == 'sample_receive') {
            $data['title'] = 'Sample Receive Voucher';
        }
        $data['pre_bal_print'] = $pre_bal_print;
        $data['user'] = $user;
        $data['hd'] = $hd;
        ...
        if ($etype == 'navigation') {
            ...
            $data['vrdetail'] = $this->purchases->fetchNavigation_receive($vrnoa, $etype, $company_id);
            $data['detail'] = $this->purchases->fetch_consume($vrnoa, $etype, $company_id);
            $balData = 0;
            $data['previousBalance'] = 0;
            $data['amtInWords'] = '';
        }elseif ($etype == 'Sample_Material_Required') {
            $data['vrdetail'] = $this->orders->fetch_req_material_($vrnoa);
            $data['detail'] = $this->orders->fetch_req_material_total($vrnoa);
            $data['rdetail'] = $this->orders->fetch_req_material_item($vrnoa);
            
            $balData = 0;
            $data['previousBalance'] = 0;
            $data['amtInWords'] = '';
        }elseif ($etype == 'issue') {
            $data['vrdetail'] = $this->purchases->fetchNavigation_issue($vrnoa, $etype, $company_id);
            $balData = 0;
            $data['previousBalance'] = 0;
            $data['amtInWords'] = '';
        }elseif ($etype == 'sample_issue') {
            ...
            $data['vrdetail'] = $this->purchases->fetchNavigation_receive($vrnoa, $etype, $company_id);
            $data['detail'] = $this->purchases->fetch_consume($vrnoa, $etype, $company_id);
            $data['details'] = $this->purchases->fetch_consumes($vrnoa, $etype, $company_id);
            $balData = 0;
            $data['previousBalance'] = 0;
            $data['amtInWords'] = '';
        } elseif ($etype == 'stocktransfer') {
            ...
            $data['previousBalance'] = 0;
            $data['amtInWords'] = '';
        } elseif ($etype == 'itv' || $etype == 'rfv' || $etype == 'rejection') {
        ...
        if ($etype == 'moulding') {
            ...
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/material_req_Voucher', $data, true));
        }elseif ($etype == 'production') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/ProductionVoucher', $data, true));
        } elseif ($etype == 'tr_produce') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/TransferVendorVoucher', $data, true));
        } elseif ($etype == 'stocktransfer') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/StockTransferVoucher', $data, true));
        } elseif ($etype == 'issue') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/IssueVoucher', $data, true));
        } elseif ($etype == 'receive') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/ReceiveVoucher', $data, true));
        } elseif ($etype == 'sample_issue') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/IssueVoucher', $data, true));
        }  elseif ($etype == 'sample_receive') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/ReceiveVouchers', $data, true));
        }elseif ($etype == 'itv' || $etype == 'rfv' || $etype == 'rejection') {
            if ($prnt == 'lg2') {
                $this->wkhtmltopdf->addPage($this->load->view('reportprints/ReceiveFromVendor', $data, true));
            } else {
    ...
    public function PrintVoucher($etype, $vrnoa, $company_id, $email_pdf = - 1, $user, $pre_bal_print, $hd = 1, $prnt = 'lg', $wrate = '1', $account) {
        $this->load->library('wkhtmltopdf');
        $data = array();
        if ($etype == 'purchase') {
            $data['title'] = 'Purchase Invoice';
        } elseif ($etype == 'issue') {
            $data['title'] = 'Stock Issuance Voucher';
        }elseif ($etype == 'receive') {
        ...
        $data['check'] = 1;
        if ($etype == 'issue') {
            $data['vrdetail'] = $this->purchases->fetch_issue_detail($vrnoa, $etype, $company_id);
            $balData = 0;
            $data['previousBalance'] = 0;
            $data['amtInWords'] = '';
        } 
        ...
        {
            $etype ='receive';
            $data['vrdetail'] = $this->purchases->fetch_receive_detail($vrnoa, $etype, $company_id);
            $balData = 0;
            $data['previousBalance'] = 0;
            $data['amtInWords'] = '';
       ...
       if ($etype == 'issue') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/IssueVoucherDetail', $data, true));
        }

        if ($etype == 'receive') {
            $this->wkhtmltopdf->addPage($this->load->view('reportprints/receiveVoucherDetail', $data, true));
        }
        $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        if ($email_pdf == 1) {
            $save_path = ($_SERVER['DOCUMENT_ROOT'] . '/AfaqTraders/application/assets/documents/' . microtime() . 'Voucher.pdf');
    ...
    public function JvVocuherPrintPdf($etype, $vrnoa, $company_id, $email_pdf = - 1, $user_print, $pr = 0, $account) {
        ...
        if ($etype == 'bpv' || $etype == 'brv') {
            if ($account == 'account') {
                ...
                $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait'));
            }
        } else {
            if ($account == 'account') {
                $data['vrdetail'] = '';
    ...
    public function CashVocuherPrintPdf($etype, $vrnoa, $company_id, $email_pdf = - 1, $user_print, $pr = 1,$prnt = 'lg',$ptype, $account = 'abc') {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['pledger'] = $this->ledgers->fetch($vrnoa, $etype, $company_id);
        if ($etype == 'cpv') {
    ...
    public function difference_reports_pdf($etype, $vrnoa, $company_id, $email_pdf = - 1, $user_print, $pr = 1,$prnt = 'lg',$ptype, $account = 'abc') {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['item'] = $this->differences->difference($vrnoa, $company_id);
        if ($etype == 'diff') {
            $data['title'] = 'Stock Transfer Difference Voucher';
        } 

        if ($pr == 1) {
            $data['header_img'] = base_url('assets/img/pic1.png/' . $this->session->userdata('header_img'));
        } 
        $this->wkhtmltopdf->addPage($this->load->view('reportprints/differencePrintPdf', $data, true));
    ...
    public function CashreqPrintPdf($etype, $vrnoa, $company_id, $email_pdf = - 1, $user_print, $pr = 1,$prnt = 'lg',$ptype, $account = 'abc') {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['pledger'] = $this->orders->fetch_detail($etype,$ptype);
        ...
        $this->wkhtmltopdf->addPage($this->load->view('reportprints/job_report', $data, true));
    ...
    public function sample_card_($etype, $vrnoa, $company_id, $email_pdf = - 1, $user_print, $pr = 1,$prnt = 'lg',$ptype, $account = 'abc') {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['pledger'] = $this->orders->job_detail_($etype,$ptype);
        ...
        $this->wkhtmltopdf->addPage($this->load->view('reportprints/card_report', $data, true));
    ...
    public function sample_card($etype, $vrnoa, $company_id, $email_pdf = - 1, $user_print, $pr = 1,$prnt = 'lg',$ptype, $account = 'abc') {
        $this->load->library('wkhtmltopdf');
        $data = array();
        $data['pledger'] = $this->orders->fetch_samplecard($etype,$ptype);
        $data['title'] = 'Sample Card Report';
        $data['user'] = $user_print;
        $data['pre_bal_print'] = 0;
        $data['check'] = 0;
        ...
        $this->wkhtmltopdf->addPage($this->load->view('reportprints/card_report_', $data, true));
    ...
    public function Print_Export_Voucher($etype, $vrnoa, $company_id, $email_pdf = - 1, $user, $pre_bal_print, $hd = 1, $prnt = 'lg', $wrate = '', $account) {
        ...
        if ($prnt == 'Commercial' || $prnt == 'Packing' || $prnt == 'BL') {
            ...
            $headerFileName = $this->pdffooter->generateHtmlFileHeader();
            $this->wkhtmltopdf->setOptions(array('orientation' => 'Portrait', 'page-size' => 'Letter', 'footer-spacing' => '5.0', 'header-spacing' => '5.0', 'footer-html' => base_url() . 'assets/temppdffooter/' . $footerFileName, 'header-html' => base_url() . 'assets/temppdffooter/' . $headerFileName, 'margin-left' => '3', 'margin-right' => '3', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        } else {
            $this->wkhtmltopdf->setOptions(array('orientation' => 'portrait', 'footer-spacing' => '3.0', 'footer-center' => '[page]/[toPage]', 'footer-font-size' => '9'));
        }
        if ($email_pdf == 1) {
