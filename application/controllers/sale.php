...
unauth_secure();
$data['modules'] = array('sale/addsale');
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->purchases->getMaxVrno('sale',$company_id) +1;
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->purchases->getMaxVrnoa('sale',$company_id) +1;
$this->output->set_content_type('application/json')->set_output(json_encode($result));
}
}
...
$result = $this->purchases->save($stockmain,$stockdetail,$vrnoa,'sale');
$response = array();
if ( $result === false ) {
$response['error'] = 'true';
}else {
...
$vrnoa = $_POST['vrnoa'];
$company_id = $_POST['company_id'];
$result = $this->purchases->fetch($vrnoa,'sale',$company_id);
$response = "";
if ( $result === false ) {
$response = 'false';
}else {
