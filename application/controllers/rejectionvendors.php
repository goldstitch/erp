...
class Rejectionvendors extends CI_Controller {
...
$data['modules'] = array('inventory/addrejectionvendors');
...
$this->load->view('inventory/addrejectionvendors',$data);
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->purchases->getMaxVrno('rejection',$company_id) +1;
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->purchases->getMaxVrnoa('rejection',$company_id) +1;
$this->output->set_content_type('application/json')->set_output(json_encode($result));
}
}
...
$vrnoa = $this->purchases->getMaxVrnoa('rejection',$stockmain['company_id']) +1;
}
$result = $this->purchases->save($stockmain,$stockdetail,$vrnoa,'rejection');
$response = array();
if ( $result === false ) {
$response['error'] = 'true';
}else {
...
$vrnoa = $_POST['vrnoa'];
$company_id = $_POST['company_id'];
$result = $this->purchases->fetch($vrnoa,'rejection',$company_id);
$response = "";
if ( $result === false ) {
$response = 'false';
}else {
