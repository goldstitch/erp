...
class FabricPurchaseContract extends CI_Controller {
...
$this->load->model('fabricPurchaseContracts');
...
$data['modules'] 	= array('contract/addFabricPurchaseContract');
...
$this->load->view('contract/addFabricPurchaseContract',$data);
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->orders->getMaxVrno('fabricPurchaseContract',$company_id) +1;
...
if (isset($_POST)) {
$company_id = $_POST['company_id'];
$result = $this->orders->getMaxVrnoa('fabricPurchaseContract',$company_id) +1;
$this->output->set_content_type('application/json')->set_output(json_encode($result));
}
}
...
$vrnoa = $this->orders->getMaxVrnoa('fabricPurchaseContract',$stockmain['company_id']) +1;
}
$result = $this->orders->save($stockmain,$stockdetail,$vrnoa,'fabricPurchaseContract');
$response = array();
if ( $result === false ) {
$response['error'] = 'true';
}else {
...
$vrnoa = $_POST['vrnoa'];
$company_id = $_POST['company_id'];
$result = $this->orders->fetch($vrnoa,'fabricPurchaseContract',$company_id);
$response = "";
if ( $result === false ) {
$response = 'false';
}else {
