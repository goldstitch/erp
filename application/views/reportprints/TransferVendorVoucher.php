...
echo '<!doctype html>
	...
	<head>
		...
		<style>
			...
			tfoot tr td { font-size: 12px !important; padding: 10px !important;  }
					...
					<div class="row-fluid relative">
						<div class="span12">
								<div class="block pull-left inv-leftblock" style="width:250px !important; display:inline !important;">
									...
									<p><span class="field ">Receive Party:</span><span class="fieldvalue cust-name">';echo ($vrdetail[0]['etype']=='tr_produce'?$vrdetail[0]['party_name']:$vrdetail[0]['party_name_consume']);;echo '</span></p>
									<p><span class="field ">Issue Party:</span><span class="fieldvalue cust-name">';echo ($vrdetail[0]['etype']=='tr_produce'?$vrdetail[0]['party_name_consume'] : $vrdetail[0]['party_name']);;echo '</span></p>
...
;echo '									<tr  class="item-row">
								...
								<tr class="foot-comments" style="border-top:0.5px solid black !important;">
									<td class="subtotal bold-td text-right" style="text-align:right !important;" colspan="5">Total:</td>
...
$netWeight=0;
foreach ($vrdetail as $row){
if($row['etype']=='tr_produce'){
$netQty += abs($row['s_qty']);
$netDozen += abs($row['dozen']);
$netWeight += abs($row['weight']);
$netPAmount += $row['s_amount'];
$netLAmount += $row['s_amount'];
;echo '									<tr  class="item-row">
									   <td class=\'text-left\'>';echo $serial++;;echo '</td>
									   <td class=\'text-left\'>';echo $row['item_id'];;echo '</td>
									   <td class=\'text-left\'>';echo $row['item_name'];;echo '</td>
								...
								<tr class="foot-comments" style="border-top:0.5px solid black !important;">
									<td class="subtotal bold-td text-right" colspan="4" style="text-align:right !important;">Total:</td>
									<td class="subtotal bold-td text-right">';echo ($netQty!=0?number_format($netQty,0):'-');;echo '</td>
									<td class="subtotal bold-td text-right">';echo ($netDozen!=0?number_format($netDozen,0):'-');;echo '</td>
									<td class="subtotal bold-td text-right">';echo ($netWeight!=0?number_format($netWeight,2):'-');;echo '</td>
