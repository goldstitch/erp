                ...
                <div class="span12" style="border:1px solid black; padding:0px;">
                    ...
                    <table class="voucher-table">
                        ...
                        <tr>
                            ...
                            <th class="numeric text-right">DPR</th>
                            <th class="numeric text-right">CTN</th>
                            <th class="numeric text-right">USD / FOB</th>
                            <th class="numeric text-right">Amount USD FOB</th>
                        </tr>
                        </thead>
                        ...
                        <tr>
                            <td colspan="2"></td>
                            <td id="tdDPR" class="text-right numeric txtbold" style="border-left:1px solid black !important;border-right:1px solid black !important;">

                            </td>
                            <td id="tdCTN" class="text-right numeric txtbold" style="border-right:1px solid black !important;">

                            </td>
                            <td class="text-right numeric txtbold" style="border-right:1px solid black !important;">
                                Total USD FOB
                            </td>
                            <td id="tdTotalCount" class="text-right numeric txtbold" style="border-right:1px solid black !important;">
...
<script type="text/javascript">
    $(function () {
        ...
        $(\'.shadowhead\').text(\'Commercial Invoice\');
        ...
        var transDD = opener.$(\'#transporter_dropdown\').select2(\'val\');
        var shipmentBy = "";
        $(\'#transporter_dropdown option\').each(function() {
            if ($(this).val().trim() == transDD) {
                shipmentBy = $(this).text();
            }
        });
        $(\'.shipmentby\').text(shipmentBy);
        ...
        opener.$(\'#purchase_table\').find(\'tbody tr\').each(function(index, elem) {
            sr = sr + 1;
            html += "<tr>";
            html += "<td><b>PO:</b> " + $.trim($(elem).find(\'td.po input\').val()) + "<br><b>QTY:</b> " + $.trim($(elem).find(\'td.qty\').text()) + "</td>";
            html += "<td>";
            if($.trim($(elem).find(\'td.ctn\').text()) != "") {
                html += "<span class=\'txtbold\'>" + $.trim($(elem).find(\'td.ctn\').text()) + " CTN CONTAINS</span> <br>";
            }
            html += $.trim($(elem).find(\'td.item_desc\').text()) + "</td>";
            html += "<td></td>";
            html += "<td class=\'numeric text-right\'>" + $.trim($(elem).find(\'td.ctn\').text()) + "</td>";
            html += "<td class=\'numeric text-right\'>" + $.trim($(elem).find(\'td.frate\').text()) + "</td>";
            var total = parseFloat($.trim($(elem).find(\'td.namount\').text()));
